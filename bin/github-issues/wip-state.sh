...
cmd_setup() {
  ...
  node(id: $projectId) {
    ...
    if [ -z "$status_field_id" ]; then
        echo "Error: 'Status' single-select field not found in project — create it in the Projects v2 UI (options: Todo / In Progress / Done)" >&2
        exit 1
    fi
    if [ -z "$fp_field_id" ]; then
        echo "Error: 'session-fingerprint' text field not found in project — create it in the Projects v2 UI (text type)" >&2
        exit 1
    fi
