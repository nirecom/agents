...
`bash "$AGENTS_CONFIG_DIR/bin/github-issues/clarify-commit-scope.sh" --session-id "<session-id>" --plans-dir "<PLANS_DIR>" --issues "$(IFS=,; echo "${closes_issues[*]}")" [--non-github]`.
...
CI-C1b. **Outline skip record**: only when intent.md satisfies BOTH so_c1 (single obvious approach) and so_c2 (change locations uniquely identified) — perform 2 SEPARATE Bash calls in order: (1) `node "$AGENTS_CONFIG_DIR/bin/workflow/record-skip-judgment" --session "$SESSION_ID" --target outline --c1 true --c2 true`; (2) `echo "<<WORKFLOW_OUTLINE_NOT_NEEDED: {reason}>>"`. No chaining. Detail skip is NOT recorded here — detail conditions cannot be evaluated before outline.md exists; defer to the make-outline-plan stage. If both conditions are not met, skip this step.
