...
CI-2b. **Companion-issue re-search.** Guards: if `closes_issues` is empty (Path C) or `NON_GITHUB=1`, skip. Flag: `bash -c 'cd "$AGENTS_CONFIG_DIR" && get-config-var --is-off CONFIRM_COMPANION_ISSUES on && echo OFF || echo ON'`. OFF = auto-append mode; ON = confirmation mode (default). Search: `bash "$AGENTS_CONFIG_DIR/bin/github-issues/find-companion-issues.sh" --primary "${closes_issues[0]}" --exclude "$(IFS=,; echo "${closes_issues[*]}")"`. The exclude list contains all current `closes_issues` entries — the same issue is never re-proposed. If stdout is empty → skip. The TSV columns are `<N>\t<title>\t<reason>\t<state>` where `reason` is a comma-separated tag list (`xref`, `ident:<tok>`, `sibling-of:#<P>`). **Confirmation (flag ON):** for each TSV line, one `AskUserQuestion`: "Add #<N> (<title>) as a related issue for this session? Reason: <reason>" — options "Yes (add as related)" / "No (skip)". **Confirmation (flag OFF):** auto-accept all candidates without prompting. Accepted `#M` appended to `closes_issues` before CI-4 writes intent.md. Note in CI-4 `## Issues` rules: companions added by CI-2b are written with the `# related` marker.
