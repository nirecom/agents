...
2. **Non-empty `closes_issues`** (skip when `NON_GITHUB=1`):
   ...
   - **WIP set for all entries.** For each issue N in `closes_issues` (primary first, then related in confirmed order):
     Before calling `wip-state.sh set <N>`, fetch `LABELS_JSON=$(gh issue view <N> --json labels --jq '[.labels[].name]' 2>/dev/null)`. If the fetch fails (exit non-zero or empty output) → warn `[clarify-intent: label probe for #<N> failed — proceeding as non-meta (WIP set will run)]` and proceed with WIP set (fail-open). If `LABELS_JSON` contains `"meta"` → emit `[clarify-intent: skipping WIP set for #<N> — meta label detected]` and skip the WIP set for this N. Otherwise:
     ...
     Exit 2 (missing env / session-id) → `AskUserQuestion` "WIP set hard failure for #<N> (rc=2: session-id or env resolution failed). WIP fingerprint absent means cross-session conflict detection is broken for this issue. How to proceed?" with two options: "Skip this N and continue (acknowledge risk)" → warn `[clarify-intent: wip-state set rc=2 for #<N> — skipped per user]` and continue; "Abort session" → emit `echo "<<WORKFLOW_ABORTED_WIP_SET_RC2: #<N>>>"` and stop.
...
3. **Empty** (Path C — skip when `NON_GITHUB=1`): `gh issue create --title "<~50 chars>" --body "<Background + Scope + Constraints + auto-created footer>" --label "intent:clarified"`. On success: backfill the `## Issues` placeholder body from `(none — pending issue creation or NON_GITHUB)` to `- #<N>: <title>` using Read + Edit (title is the `--title` arg from this call — no re-fetch needed). Then apply the same meta-skip + rc=2 escalation as the WIP-set-for-all-entries loop above: fetch `LABELS_JSON=$(gh issue view <N> --json labels --jq '[.labels[].name]' 2>/dev/null)` for the freshly created N. If fetch fails → warn `[clarify-intent: label probe for #<N> failed — proceeding as non-meta (WIP set will run)]` and proceed. If `LABELS_JSON` contains `"meta"` (edge case for a freshly created issue) → emit `[clarify-intent: skipping WIP set for #<N> — meta label detected]` and skip the WIP set. Otherwise call `bash "$AGENTS_CONFIG_DIR/bin/github-issues/wip-state.sh" set <N>` with rc=1 → warn-and-continue; rc=2 → `AskUserQuestion` "WIP set hard failure for #<N> (rc=2: session-id or env resolution failed). WIP fingerprint absent means cross-session conflict detection is broken for this issue. How to proceed?" with options "Skip this N and continue (acknowledge risk)" → warn and continue; "Abort session" → emit `echo "<<WORKFLOW_ABORTED_WIP_SET_RC2: #<N>>>"` and stop. Then `bash "$AGENTS_CONFIG_DIR/bin/github-issues/ensure-board-card.sh" <N>` (best-effort). On `gh issue create` failure: warn, leave the `## Issues` placeholder body unchanged.
