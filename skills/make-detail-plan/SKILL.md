...
5. **Review the draft with codex first, fall back to Claude if unavailable.**
   ...
   c. Run via Bash: `review-plan-codex --input ~/.workflow-plans/drafts/<session-id>-detail-draft.md --format detail-plan [--context ~/.workflow-plans/drafts/<session-id>-context.md] --context "$AGENTS_CONFIG_DIR/rules/core-principles.md"`
...
- Follow `rules/core-principles.md` — §1 (elevate perspective) before drafting, §2 (orthogonality) for cross-platform / symmetric coverage, §3 (name reflects substance) for any new file or symbol names introduced by the plan
