...
Dispatch `doc-append` per `skills/_shared/worker-dispatch.md`. Payload: `mode: "compose"`, `notes_path: NOTES_BACKUP_PATH`, `branch`, `pr_number`, `merge_commit: MERGE_SHA`, `pr_title`, `closes_issues_count: CLOSES_ISSUES_COUNT`, `cwd: MAIN_ROOT`, `artifact_dir: PLANS_DIR`.
