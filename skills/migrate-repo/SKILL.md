...
MR-1. Get target repo path from user. Do NOT silently infer REPO_PATH from a path mentioned as documentation/reference/context — accept only the path the user explicitly designates as the migration target.
   - Resolve REPO_PATH to an absolute path. AskUserQuestion: "Migration target = `<resolved absolute path>` — correct? (confirm this is NOT the agents repo itself)" Options: "Yes, proceed with this path" / "No, re-enter the correct path". On "No": re-collect REPO_PATH.
MR-2. Preview + capture snapshot: `eval "$(bash "$AGENTS_CONFIG_DIR/skills/migrate-repo/scripts/preview-and-capture.sh" "$REPO_PATH")"`. Runs dry-run, mirrors output to stderr, and exports `MIGRATE_ACK_UP_TO_ISSUE_N` + `MIGRATE_ACK_SELF_COUNT_AT_ACK`.
   - After eval, inspect MIGRATE_SELF_REPO_DETECTED. If =1: AskUserQuestion "WARNING: target path (<REPO_PATH>) is identical to the agents repo itself (AGENTS_CONFIG_DIR). Choose 'proceed' ONLY for an intentional agents-repo Phase 3 self-migration; otherwise choose 'abort' and re-enter the correct path." Options: "Proceed as agents-repo Phase 3" / "Abort and fix the path". On abort: stop the skill. On proceed: continue to MR-3. The eval also exports MIGRATE_ACK_UP_TO_ISSUE_N and MIGRATE_ACK_SELF_COUNT_AT_ACK.
...
MR-4. **History canary 1**: `MIGRATE_ACK_EXISTING_ISSUES=1 MIGRATE_ACK_UP_TO_ISSUE_N="$MIGRATE_ACK_UP_TO_ISSUE_N" MIGRATE_ACK_SELF_COUNT_AT_ACK="$MIGRATE_ACK_SELF_COUNT_AT_ACK" bash "$AGENTS_CONFIG_DIR/bin/github-issues/migration/orchestrate.sh" "$REPO_PATH" --from-step 1 --stage canary-1`
