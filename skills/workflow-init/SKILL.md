...
- PM1. `node "$AGENTS_CONFIG_DIR/bin/workflow/set-workflow-type" --session "$SESSION_ID" --type wf-meta --advance --step workflow_init --status complete` (single Bash call — records `workflow_type` and completes `workflow_init` together; no `--next` since the next action is fixed at PM2).
