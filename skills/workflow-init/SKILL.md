...
Canonical: `skills/_shared/non-github-remote-gate.md`. `NON_GITHUB=1` → skip Steps WI-3..WI-9 (issue detection / `gh issue view` / route logic), proceed as **Path C**. Steps WI-10..WI-13 (context.md write, survey launch, C1–C2) run as normal.
...
### Step WI-5 — Companion-issue detection
...
**Search:** `bash "$AGENTS_CONFIG_DIR/bin/github-issues/find-companion-issues.sh" --primary "${ISSUES[0]}" --exclude "$(IFS=,; echo "${ISSUES[*]:1}")"`. If stdout is empty → skip rest of WI-5.
...
**Acceptance:** Append each accepted `#M` to `ISSUES` array. These entries flow into WI-6 aggregate WIP check and WI-13 automatically.

### Step WI-6 — Aggregate WIP check (OPEN branch)
...
### Step WI-7 — CLOSED detection (post-WIP)
...
### Step WI-11 — Parallel survey launch (all Paths)
...
### Step WI-12 — Post-check

Apply `skills/_shared/survey-artifact-valid.md` to each artifact. On invalid: emit `<<WORKFLOW_SURVEY_AGENT_FAILED: survey-code>>` or `<<WORKFLOW_SURVEY_AGENT_FAILED: survey-history>>`. Fall through to WI-13 on failure — do NOT abort. `clarify-intent` handles missing-or-invalid artifacts.

### Step WI-13 — Path-specific steps
...
- B1. Write `<PLANS_DIR>/drafts/<session-id>-issue-prefill.md` with `<!-- Issue #<N> seed for clarify-intent. Confirm framing, do not start from scratch. -->`, `# Issue #<N>: <title>`, `<body>`. If WI-5 accepted companion issues, append one section per companion: `\n\n## Companion #<M>\nRelated: #<M>` so `clarify-intent` CI-1a's `#\d+` scan detects them.
