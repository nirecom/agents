...
8. Write the chosen approach to `<PLANS_DIR>/<session-id>-outline.md` per the Output Schema. Apply the full `skills/_shared/confirm-plan.md` protocol (Steps 1+2+3) using `CONFIRM_OUTLINE`. On the `ON` path: emit `echo "<<WORKFLOW_CONFIRM_OUTLINE: <one-line summary>>>"` per protocol Step 3 — co-emit <<WORKFLOW_MARK_STEP_outline_complete>>, <<WORKFLOW_OUTLINE_PLAN_COMPLETE>>, AND the make-detail-plan skill invocation in the same response (see confirm-plan.md Step 3 co-emission). Revise → ask what to change, re-run outline-planner, loop back to Step 7.
