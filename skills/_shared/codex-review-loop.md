...
- Optional: `CTX_SURVEY_CODE`, `CTX_SURVEY_HISTORY` — each passed as `--context` when the file exists and is non-empty.
- `CTX_CONCERNS_LOG` — auto-generated by each wrapper via `concern-ledger render-concerns-log`; do not set it from the caller (#2185).
