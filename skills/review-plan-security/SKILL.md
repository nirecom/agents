...
- exit 1 NEEDS_REVISION → terminal; save stdout to `<PLANS_DIR>/<session-id>-security-plan-codex-round-<N>-raw.md` (`<N>` from `<PLANS_DIR>/<session-id>-security-plan-last-round.txt`); present concerns with per-axis severity; propose mitigations; stop (no re-loop) → RPS-3.
- exit 2 ESCALATE → run `review-loop-summarize-concerns --budget-remaining 0`; present summary; stop → RPS-3.
- exit 6 HIGH_UNRESOLVED → save stdout to `<PLANS_DIR>/<session-id>-security-plan-codex-round-<N>-raw.md` (`<N>` from `<PLANS_DIR>/<session-id>-security-plan-last-round.txt`); run `review-loop-summarize-concerns --budget-remaining 0`; present unresolved HIGH concerns; stop → RPS-3.
