...
### Step 2 — Cross-session resume (`--from` / `--list`)

Route here on the arguments alone, before any local detection: the user passed `--from <session-id>`, passed `--list`, or named a session that is not this one's ancestor.
...
This route ends the skill at Step 5 — never fall through to local detection.

### Step 3 — Detect

Enter only when Step 2 did not claim the invocation: a `--from` or `--list` run never reaches local detection, whose `none` verdict would stop the very resume it was asked for.
Run via Bash:
```
node "$AGENTS_CONFIG_DIR/bin/resume-session-detect"
```
Parse stdout as JSON. Dispatch by `type` in Step 4.
...
- Read workflow state only, except in Step 2, where `--from` adopts into this session.
