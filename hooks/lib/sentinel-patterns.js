...
const REVIEW_TESTS_WARNINGS_ACCEPTED_RE_DQ = /^echo "<<WORKFLOW_REVIEW_TESTS_WARNINGS_ACCEPTED: ([^>()\x60;|&]+)>>"$/;
const REVIEW_TESTS_WARNINGS_ACCEPTED_LOOKSLIKE_RE = /^echo "<<WORKFLOW_REVIEW_TESTS_WARNINGS_ACCEPTED([: ].*)?>>"\s*$/;
...
function isSentinel(cmd) {
  return (
    ...
    REVIEW_TESTS_WARNINGS_ACCEPTED_RE_DQ.test(cmd) ||
    REVIEW_TESTS_WARNINGS_ACCEPTED_LOOKSLIKE_RE.test(cmd) ||
