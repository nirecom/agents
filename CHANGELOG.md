...
Changes: ENFORCE_WORKTREE=on (default) blocks all Edit/Write/Bash-write operations from the main checkout regardless of branch — work must happen in a linked worktree (/worktree-start). Worktrees follow a standard <WORKTREE_BASE_DIR>/<task>/<repo> path layout. /worktree-end now requires PR + merge as the only exit path. Installer adds a global gitignore block for WORKTREE_NOTES.md (idempotent on Linux/macOS/Windows). Set ENFORCE_WORKTREE=off in agents config to opt out for trivial direct-main work. Deprecation: previous variable names AGENT_AUTO_BRANCH and AGENT_DEFAULT_BRANCHES are still accepted for backward compatibility but emit a deprecation warning — rename to ENFORCE_WORKTREE / DEFAULT_BRANCHES in your agents config.
