...
- Under `ENFORCE_WORKTREE=on`, `/update-docs` stages bullets into `WORKTREE_NOTES.md` `## History Notes`. `bin/compose-doc-append-entry` appends them to `history.md` from the main worktree during `/worktree-end` Step 6i.
- Format:
  ```
