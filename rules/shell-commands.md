...
|------|---------------|-------|
| <redacted> (Windows) | **pwsh (PowerShell)** | `curl.exe` required; WSL sessions use bash/Linux as normal |
| <redacted> (QNAP) | **bash** | No curl/wget — use Python method below |

When suggesting verification commands for <redacted>, default to **pwsh-compatible commands**.
...
QNAP (<redacted>) and most Docker containers do **not** have `curl` or `wget`.
