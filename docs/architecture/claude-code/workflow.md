...
Output is four `KEY=value` lines: `ACTION` (`invoke|judgment_needed|done|blocked|abort`), `NEXT_SKILL`, `NEXT_HINT`, `REASON`. The `NEXT_SKILL` field maps directly to a skill name; non-skill steps (e.g. `branching_complete`, `user_verification`) have an empty `NEXT_SKILL` and a prose `NEXT_HINT` instead.

At the `outline` and `detail` steps, next-step emits `ACTION=judgment_needed` (rather than `invoke`) to signal a semantic skip judgment opportunity. `NEXT_HINT` contains the path to read and the judgment rubric: intent.md when judging the outline step or when outline was skipped (C1 fallback); outline.md when judging the detail step with a completed outline. The orchestrator reads the artifact and decides: if skip is warranted, emit `<<WORKFLOW_OUTLINE_NOT_NEEDED: judgment=true: {reason}>>` or `<<WORKFLOW_DETAIL_NOT_NEEDED: judgment=true: {reason}>>` — the `judgment=true` prefix causes `gate-plan-skip-sentinel.js` to unconditionally allow the sentinel before any `CONFIRM_*` check fires; otherwise launch `NEXT_SKILL` as normal.

At the `outline` and `detail` steps only, next-step additionally appends an optional fifth line `SKIP_HINT` (`WORKFLOW_OUTLINE_NOT_NEEDED` or `WORKFLOW_DETAIL_NOT_NEEDED`) when the session's `intent.md` reads as trivial (a mechanical-change keyword present, no broad-change or new-API-surface signal). It is advisory only — a suggestion that the model may emit the corresponding ask-gated skip sentinel or ignore; the four-line contract is unchanged on every other step. Triviality is judged by `hooks/lib/workflow-state/skip-signal-resolver.js` (`isTrivial`), which fails closed to "not trivial" on any uncertainty.
