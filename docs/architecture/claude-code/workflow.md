...
Edit/Write/MultiEdit/editFiles/NotebookEdit attempt → workflow-gate.js (PreToolUse hook, early gate)
  ...
  allowlist: Write to ~/.workflow-plans/** (resolved via getWorkflowPlansDir(); honours WORKFLOW_PLANS_DIR override) is permitted (clarify-intent skill writes intent.md/outline.md/detail.md here)
