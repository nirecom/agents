...
| `hooks/postuse-step-in-flight-mark.js` | TL2 (`tests/feature-2013-step-in-flight-automark.sh` + `d-skill-dispatch.sh` / `e-lookahead-guard.sh` — payload injection: allow-list, lookahead-window, `META_OP_SKILLS` exclusion) + TL3 (`tests/TL3-hook-skill-dispatch-payload.sh` — RUN_TL3-gated) | **TL3 gated** | TL2 pins the matching/exclusion logic by synthetic `PostToolUse` payloads; only a live session proves the real `tool_input.skill` field name and its dotted/slashed segment shape match what `skillNameOf` expects — a mismatch there would silently defeat the `META_OP_SKILLS` fence (#2279). |
