...
| `hooks/stop-confirm-plan-guard.js` | **TL3 covered** (`tests/TL3-hook-stop-confirm-plan-guard.sh` — RUN_TL3-gated; TL2: `tests/fix-524-confirm-plan-guard.sh` T13–T35 pin Layer 2 marker-independence and the PLAN_LANG re-lint, #2278) | done (#943) | Turn marker fixture consumed (deleted) by the live Stop hook via readAndDeleteTurnMarkers(). Since #2278 Layer 2 (CONFIRM follow-up + artifact re-lint) fires on every Stop without a marker; the TL3 seam still only exercises the marker-gated Layer 1 path. |
| `hooks/gate-plan-lang.js` | TL2 (`tests/feature-2278-pretool-lang-gates.sh` — PLG-T1..T32: per-fragment lint, per-element paths, `..` resolution, fail-open matrix, read-only/idempotent, ~200KB stdin) | **TL3 gap** | hook-registration gap: real PreToolUse firing and the CC runtime actually rejecting a Write/Edit/MultiEdit/editFiles call before the bytes land is unverifiable at TL2 (#2278). |
| `hooks/gate-worktree-notes-lang.js` | TL2 (`tests/feature-2278-pretool-lang-gates.sh` — WNG-T1..T25: public/private routing via fixture repos, heading-bearing vs reconstructed fragments, fail-open, no cross-path aggregation) | **TL3 gap** | Same hook-registration gap as `gate-plan-lang.js`; private-repo routing additionally depends on the cwd the runtime hands the hook, which only a live session pins (#2278). |
