...
| **code-side null** | A TR5 `freshness_key == null` case where `input_version` is also null (no merge base, shallow clone, or detached HEAD). `selfRecovering` approves a prior non-BLOCK CONTINUE terminal; otherwise a full re-audit is armed without `recurrence-patterns` (its input key is always null here). Introduced in #2323. | [claude-code/supervisor-audit-ledger.md](architecture/claude-code/supervisor-audit-ledger.md) |
| **artifact-side null** | A TR5 `freshness_key == null` case where `input_version` is non-null (diff is computable but the plan artifact — intent/outline/detail — is missing from the filesystem). `selfRecovering` approves when `trigger_input_keys.TR5` stored in the last terminal run matches the current `input_version`; arm without `recurrence-patterns` otherwise. Introduced in #2360. | [claude-code/supervisor-audit-ledger.md](architecture/claude-code/supervisor-audit-ledger.md) |
