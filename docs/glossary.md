...
| **trigger** | The general term for a condition that arms the supervisor. Audit triggers are step completion (5 steps) or the severity threshold; alert triggers are sentinel-hang / scheduled-review / off-proposal. Do not call a trigger an "anchor". | [claude-code.md](architecture/claude-code.md) |
