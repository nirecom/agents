...
- [ ] Phase 4b: dotfiles / my-private-repo 横展開（`.github/` + `bin/sync-labels.sh`）
...
### worktree-start でのファイルコピー制限緩和（B-Hybrid Phase 1 実装済み）

`.env` / `.private-info-allowlist` 等の main → worktree コピーを `bin/worktree-copy-include.js` + `.worktreeinclude` / `.worktreecopyexclude` で実装。Phase 2 (B-Pure) は別エントリ。
...
### enforce-worktree: main worktree から `git push origin main` がブロックされる

main worktree CWD でドキュメントを直接コミット (`ENFORCE_WORKTREE_EXCLUDE` 経由で pre-commit は通る) した後、`git -C C:/git/agents push origin main` が enforce-worktree.js (PreToolUse Bash) にブロックされる。commit は通すが push は通さない非対称構造のため、commit 直後の自動 push ができずユーザが手動で push する必要がある。

**症状**: docs-only コミット直後の `git push origin main` で `ENFORCE_WORKTREE: write blocked. Reason: main worktree (branch 'main').` が出る。
...
`/worktree-end` Step 6c (`git worktree remove`) で worktree が git 管理から外れた後、残ったディレクトリ (`<WORKTREE_BASE_DIR>/<task-name>/`) を `rm -rf` / `Remove-Item -Recurse -Force` で削除しようとすると、main worktree CWD から enforce-worktree.js にブロックされる。`Bash(*rm -rf *)` と `Bash(*Remove-Item*-Recurse*-Force*)` は settings.json deny でもあるため allow rule では解決しない。

`isAllowedNewItemDirectory` (外部パス `New-Item -ItemType Directory`) の exemption はあるが、逆方向（外部パスの再帰削除）に相当する exemption がない。結果として `/worktree-end` 完了後にユーザがターミナルから手動削除する必要がある。

**対応候補**:
- [ ] `enforce-worktree.js` に `isAllowedWorktreeDirectoryRemove` を追加: 対象パスが `WORKTREE_BASE_DIR` 配下かつ git worktree list に存在しない（既に de-register 済）であれば pass through
- [ ] settings.json の `rm -rf` / `Remove-Item -Recurse -Force` deny rule を `WORKTREE_BASE_DIR` 配下に限り narrow exception を入れる
- [ ] `/worktree-end` SKILL.md に「最後の手動削除コマンド」を明示する短期回避策を追加

### planning skills の CONFIRM_* チェックが main worktree から実行できない

発見: PR #35 (`fix/settings-getconfig`) の作業中に確認。

`clarify-intent` / `make-outline-plan` / `make-detail-plan` は最終ステップで `CONFIRM_OUTLINE` / `CONFIRM_DETAIL` フラグを `bash -c 'get-config-var --is-off ...'` で確認するが、これらの skill が走るのは worktree 作成**前**＝main worktree CWD のため、`enforce-worktree.js` PreToolUse フックが Bash ツール全体をブロックする（read/write 区別なし）。settings.json の allow rule はフックより後に評価されるため allow rule 追加では解決しない。

**影響**: `CONFIRM_*=off` 設定が main worktree 起点の skill では効かず、常に `on` 扱いになる（プラン承認プロンプトが skip されない）。

**今回の Bug②（`cd "$AGENTS_CONFIG_DIR" &&` プレフィックス）との違い**:
- Bug②: linked worktree 内から呼ぶときに `.env` を見つけられない問題（CWD 依存）→ 解決済
- 本件: main worktree から呼ぶときに Bash 全体が enforce-worktree でブロックされる問題（フック設計）

**対応候補**:
- [ ] `enforce-worktree.js` に read-only Bash コマンドの allowlist を追加（`get-config-var --is-off *` 等を pass through）
- [ ] 各 planning skill の CONFIRM_* チェックを Bash から Node hook 経由に置き換え（フック評価より前に動作）
- [ ] CONFIRM_* チェックの実行タイミングを worktree 作成後にずらす（skill 設計変更）
