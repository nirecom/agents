...
### enforce-worktree: main worktree から `git push origin main` がブロックされる **[Verifying]**

main worktree CWD でドキュメントを直接コミット (`ENFORCE_WORKTREE_EXCLUDE` 経由で pre-commit は通る) した後、`git -C C:/git/agents push origin main` が enforce-worktree.js (PreToolUse Bash) にブロックされる。

**実装** (`fix/enforce-worktree-push-fix`): `isAllowedPushAllExcluded(cmd, repoRoot, excludePatterns)` を `enforce-worktree.js` に追加。outgoing range (`<upstream>..HEAD`) の全コミット全ファイルが `git log --name-only` で列挙され、全て EXCLUDE 対象なら pass through。refspec 不正形（`:`、`refs/`、`+`、複数 refspec）は fail-closed。

**動作確認待ち**:
- [ ] main worktree から docs-only commit → `git push origin main` が通ること
- [ ] 非 EXCLUDE ファイルを含む commit を作って push がブロックされること

### worktree-end: orphan worktree ディレクトリの再帰削除が enforce-worktree にブロックされる **[Verifying]**

`/worktree-end` Step 6c (`git worktree remove`) で worktree が git 管理から外れた後、残ったディレクトリ (`<WORKTREE_BASE_DIR>/<task-name>/`) を `rm -rf` / `Remove-Item -Recurse -Force` で削除しようとすると、main worktree CWD から enforce-worktree.js にブロックされる。

**実装** (`fix/enforce-worktree-push-fix`): 専用スクリプト `hooks/cleanup-orphan-dir.js` を新設。`fs.rmdirSync` (非再帰) で空ディレクトリのみ削除。シンボリックリンク・登録済み worktree・`WORKTREE_BASE_DIR` 外・非空・base 等価は全て refuse (fail-closed)。`/worktree-end` SKILL.md Step 6e がこのスクリプトを呼ぶように更新済み。settings.json に明示的 allow rule 追加。
...
`clarify-intent` / `make-outline-plan` / `make-detail-plan` は最終ステップで `CONFIRM_OUTLINE` / `CONFIRM_DETAIL` フラグを `bash -c 'get-config-var --is-off ...'` で確認するが、これらの skill が走るのは worktree 作成**前**＝main worktree CWD のため、`enforce-worktree.js` PreToolUse フックが Bash ツール全体をブロックする。

**実装** (`fix/enforce-worktree-push-fix`): `bash-write-patterns.js` の `classify()` を拡張し、`bash -c '...'` / `pwsh -Command '...'` の inner body を `&&`/`||`/`;` で分割して各セグメントを再分類。全 segment が "read" なら "read" として fast-path → main worktree からも通過する。outer chaining (`bash -c '...' && rm`) は `stripQuotedArgs` 後の chaining 検出で fail-closed。改行を含む inner body も fail-closed。

**動作確認待ち**:
- [ ] main worktree から planning skill 実行時 `CONFIRM_*=off` が反映されること
- [ ] `bash -c 'rm -rf /'` 等の write が引き続きブロックされること
