...
### enforce-worktree: `isAllowedWorktreeCommand` が `git worktree add` をブロックする

**現象**: `git worktree add <outside-path> -b <branch>` を Bash tool から実行すると、main checkout 検出後の例外チェック (`isAllowedWorktreeCommand`) が true を返すはずなのにブロックされる。

**推定原因**: `isPathOutsideRepo` の中で `repoRoot` に `git rev-parse --show-toplevel` の出力（Git Bash 形式: `/c/git/repo`）をそのまま `path.resolve()` に渡している。Windows の Node.js は `/c/git/repo` を `C:\git\repo` ではなく `C:\c\git\repo` に解決するため、比較が狂って outside 判定が誤る可能性がある。

**再現条件**: Windows + Git Bash 環境で `git worktree add` を Bash tool から実行。

**解決候補**:
- [ ] `isPathOutsideRepo(targetPath, repoRoot)` の `repoRoot` にも `normalizeCwd`（Unix drive path → Windows 変換）を適用する
- [ ] または `spawnSync` で `--show-toplevel` の代わりに `--absolute-git-dir` / Windows-native パスを得る方法を調査

### bash-write-patterns: `2>/dev/null` が write に誤分類される — Known Issue

**現象**: `2>/dev/null`（エラー抑制用リダイレクト）が stderr-redirect パターン（`\d>`）にマッチして write 分類され、main checkout から enforce-worktree にブロックされる。`/dev/null` は実ファイルへの書き込みではないが、現在のパターンは区別しない。`>/dev/null` も同様。

**影響範囲**: worktree-end の `docker ps 2>/dev/null` 等、エラー抑制リダイレクトを含む読み取り系コマンド。

**解決候補**:
- [ ] `/dev/null` へのリダイレクト（`2>/dev/null`, `>/dev/null`）を read として扱う例外を追加
- [ ] FD-to-FD リダイレクト（`2>&1`, `1>&2`）も documented FP → read 化を検討
