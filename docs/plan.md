...
`c:\LLM\ai-specs\projects\engineering\langchain\history.md` に
`#2: Worktree 削除で .env 消失、Langfuse ボリューム復旧不能 (2026-04-28)` を `doc-append` で追加。

```bash
doc-append c:/LLM/ai-specs/projects/engineering/langchain/history.md \
  --category INCIDENT \
...
| `c:\LLM\ai-specs\projects\engineering\langchain\history.md` | INCIDENT #2 追加（`doc-append`） | doc |
...
| INCIDENT #2 追記 | `tail -30 c:/LLM/ai-specs/projects/engineering/langchain/history.md` | `### #2: ...` が末尾に存在、`Cause:`/`Fix:` 行あり |
