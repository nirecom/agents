...
- Follow `rules/core-principles.md`. Read `rules/coding.md` before following it — it is on-demand-only and never auto-injected.
- Read `rules/test.md` and `rules/docs.md` before following them — they are on-demand-only and never auto-injected.
...
Before reading Class members, count `## Issues` entries and `## Class members` entries in the provided intent.md (Class members SSOT is intent.md, #2228). When issues_count > 0 and members_count < issues_count, note this gap explicitly in the plan's `## Risks & edge cases` section.
...
**Anti-pattern (`rules/core-principles.md` CPR-E2C violation):** Covering only one
MUST member while ignoring the others. If the user has to enumerate each one
for you, you failed CPR-E2C.
