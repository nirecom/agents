...
- Step L: read `triage_action` from state. Determine `history_entry_status`: `"skipped_no_history_notes"` when `triage_action == auto_close_path` (PR used `closes #N` keyword; no WORKTREE_NOTES.md was generated by this session); `"written_by_step_6h"` otherwise (normal worktree path: `/worktree-end` Step 6h is the canonical writer of `docs/history.md`, #690). Then: `node "$agents_config_dir/bin/issue-close-write-outcome.js" --session-id "$session_id" --out-file "$outcome_file_path" "$current_issue_number" "succeeded" "$history_entry_status" "succeeded" "$j_status" "$k_status"`. Non-zero → log warning; continue (non-fatal).
