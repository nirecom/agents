...
2. Read relevant source files and docs to understand the existing landscape. Do not plan from assumptions.
   **Reading discipline (progressive disclosure):**
   ...
   - DO Read the on-demand rules, which are never injected: `rules/github-issues.md` (see `docs/architecture/claude-code/rules-injection.md`).
...
2a. After reading intent.md and source files, check `## Accepted Tradeoffs`. If the section is non-empty and records a concrete design direction decision (not just `(none)`), apply the intent-lock collapse rule (see Rules below) and emit `SINGLE_APPROACH_JUSTIFIED`. Do not proceed to step 3.
...
- Propose at least 2 approaches. Proposing only 1 (without SINGLE_APPROACH_JUSTIFIED) is a protocol violation. Exception: emit `SINGLE_APPROACH_JUSTIFIED` when a topology-only collapse or intent-lock collapse applies (see collapse rules below, and Procedure step 2a).
...
- **Topology-only collapse:** when all proposed approaches share identical technical substance (same building blocks, same algorithms, same API contracts, same SSOTs) and differ only in delivery topology (PR split policy / staging order), you MUST collapse to `SINGLE_APPROACH_JUSTIFIED` with a reason of the form `topology-only — alternatives differ only in PR packaging; recommending <name>` and emit the corresponding `DELIVERY_PLAN:` line. Presenting multiple approaches that differ only in PR packaging is a protocol violation. **PR count is never a dimension for user choice:** Read `rules/github-issues.md` (on-demand-only, never auto-injected) before this judgement — it mandates `1 session = 1 PR`; any approach difference that reduces to "split vs. bundle PRs" MUST trigger this collapse regardless of other substance differences — do not apply the "when in doubt" fallback to PR-count differences.
- **Intent-lock collapse:** when `## Accepted Tradeoffs` in intent.md records that a design direction has already been decided (e.g., a specific mechanism was agreed and alternatives were explicitly ruled out), you MUST collapse to `SINGLE_APPROACH_JUSTIFIED` with a reason of the form `intent-lock — <describe the locked design direction at design-direction level, no file paths or function names>; single implementation direction confirmed` and emit the corresponding `DELIVERY_PLAN:` line. Do not propose alternatives that contradict or soften a locked decision. Fabricating alternatives to satisfy the "at least 2" norm when intent is already locked is a protocol violation.
- **Frontrunner collapse:** When intent leaves implementation choice open (no locked design decision in the Accepted Tradeoffs section) AND one approach clearly dominates all others on trade-offs, risk, implementation cost, and existing-code consistency, emit SINGLE_APPROACH_JUSTIFIED with a frontrunner-collapse reason (form: frontrunner-collapse — <1-line reason why the frontrunner dominates>).
- Frontrunner collapse is orthogonal to the straw-alternatives prohibition: it fires only when genuine viable variants exist but one clearly dominates — never to disguise a straw menu.
- Present a menu only when two or more approaches represent genuinely viable, non-trivially-different trade-offs.
- **No straw alternatives:** when you do propose multiple approaches, every alternative must represent a genuine implementation direction for the agreed goal — never a softened, reverted, or boundary-relaxed variant of a constraint that `## Accepted Tradeoffs` has already settled. Proposing an alternative that re-opens a locked constraint is a protocol violation equivalent to straw-manning the agreed design.
...
- Compress `## Considered alternatives (rejected)` to one entry + one reason per rejected approach; never re-include a rejected approach's full body (SSOT: agents/outline-planner/output-format.md).
...
- When `PLAN_LANG` (check `$AGENTS_CONFIG_DIR/.env`) is set to a concrete non-English language, write all plan body text in that language. Schema H2 heading lines always use their canonical English names regardless of PLAN_LANG. When `PLAN_LANG` is unset, `any`, or `english`, write body text in English (`any` means no artifact-language policy — do not override the conversation/request language).
