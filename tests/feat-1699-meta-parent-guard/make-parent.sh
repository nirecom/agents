...
#
# Group B — make-parent creates TWO issues
#
# Contract under test: `make-parent` no longer promotes the proposal itself into the
# parent slot. It creates (1) a meta-labeled `Group: `-titled parent whose body is
# generated by the dispatcher, and (2) the proposal, unmodified, as a child. Existing
# siblings named by --children plus the new child all attach under the new parent.
# stdout is two lines, parent FIRST and child LAST, so that every caller doing
# `| tail -n 1` keeps receiving the proposal's URL.

# arg_lines_after <call-file> <flag> <n> → the n argv LINES following <flag>.
# `arg_after` stops at one line, which is right for a title and wrong for a body: the
# mock writes one argv element per line, so a two-line body occupies two lines and a
# one-line read would compare half a body and call it verbatim.
arg_lines_after() {
    awk -v flag="$2" -v n="$3" '
...
# The prefix check above passes for `Group: ` + anything. The normalisation is a pure
# function of the proposal title, so the whole result is pinned: a parent titled
# "Group: " with the subject dropped, truncated, or re-worded would satisfy B2b.
...
# "Verbatim" is the actual contract, and only a whole-body comparison states it. The
# dispatcher rewrites bodies elsewhere (inject_related_into_body for `sibling`), so
# "the child body is not the parent's" (B4) does not rule out a rewritten child body.
...
# The meta label belongs to the container, not to the work. A child carrying it would
# be picked up as an eligible parent by require-meta-parent.sh's first condition.
...
# cbody must be non-empty too: with only one create call there is no child file, and
# an empty $cbody would make the inequality trivially true.
...
# The parent is a container. Saying so in the body is what stops a future reader from
...
# WHICH ids were attached, not merely how many. The count alone passes if the dispatcher
# attached #42 three times, or attached the parent to itself. The mock's databaseId is
# the issue number followed by "000", so the expected set is exact: the two named
# children (42, 43) plus the NEWLY CREATED proposal (202) — never the parent (201).
...
# Stated separately because it is the one id whose presence would be silently plausible:
# a self-attach reads as "3 attaches under #201" to every count-based assertion.
