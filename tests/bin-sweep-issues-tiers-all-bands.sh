...
# Tags: sweep, issues, all-bands, snapshot, scope:common, TL2, dup-group-keep:size-hard-limit
# Split from bin-sweep-issues-tiers.sh (500-line HARD limit): pins the --all-bands
# snapshot sweep — one issue-list fetch, per-band SI-2 scan, a single tier-1 pass,
# one aggregated tier-2 gate, band_index=all CI contract, --max-bands, and fail-fast
...
# ── C12 (B1): --all-bands sweeps every band in one run ───────────────────────
C12_all_bands_sweeps_all() {
    make_fixture c12
    local out; run_sweep --all-bands --band-size 2; out="$OUT"
    [ "$RC" -eq 0 ] \
        && pass "C12 --all-bands: exit 0" \
        || fail "C12 --all-bands: exit=$RC (want 0)"
    echo "$out" | grep -q 'scanned=4' \
        && pass "C12 --all-bands: summary reports scanned=4 (all 4 issues)" \
        || fail "C12 --all-bands: scanned!=4, out=$out"
    # #101 is in band 0, #103 in band 1: both bands must have been scanned.
    echo "$out" | grep -q 'TIER2-CANDIDATE: issue=101' \
        && pass "C12 --all-bands: band 0 candidate #101 reported" \
        || fail "C12 --all-bands: no TIER2-CANDIDATE for #101 (band 0), out=$out"
    echo "$out" | grep -q 'TIER2-CANDIDATE: issue=103' \
        && pass "C12 --all-bands: band 1 candidate #103 reported" \
        || fail "C12 --all-bands: no TIER2-CANDIDATE for #103 (band 1), out=$out"
    echo "$out" | grep -qF 'TIER2-CANDIDATE: issue=102' \
        && fail "C12 --all-bands: #102 is live (existing.sh exists) — must NOT be a candidate" \
        || pass "C12 --all-bands: #102 correctly absent from candidates"
    echo "$out" | grep -qF 'TIER2-CANDIDATE: issue=104' \
        && pass "C12 --all-bands: #104 is stale (gone-d.sh missing) — correctly a candidate" \
        || fail "C12 --all-bands: #104 missing from candidates, out=$out"
}

# ── C13 (B2): snapshot — the issue list is fetched exactly once ──────────────
C13_all_bands_single_fetch() {
    make_fixture c13
    run_sweep --all-bands --band-size 2
    local n; n="$(gh_band_fetch_count)"
    [ "${n:-0}" -eq 1 ] \
        && pass "C13 --all-bands: gh issue list (band fetch) called exactly once" \
        || fail "C13 --all-bands: band fetch ran ${n:-0} times (want 1); gh calls=$(tr '\n' '|' < "$GHREC")"
}
...
C14_all_bands_tier1_once() {
    ...
    [ "${n:-0}" -eq 1 ] \
        && pass "C14 --all-bands: parent-all-closed-check #900 ran exactly once" \
        || fail "C14 --all-bands: tier-1 check #900 ran ${n:-0} times (want 1); record=$(tr '\n' '|' < "$RECORD")"
    local c; c="$(grep -cE '^close-completed 900' "$RECORD" 2>/dev/null || true)"
    [ "${c:-0}" -eq 1 ] \
        && pass "C14 --all-bands: close-completed #900 executed exactly once" \
        || fail "C14 --all-bands: close-completed #900 ran ${c:-0} times (want 1); record=$(tr '\n' '|' < "$RECORD")"
}

# ── C15 (B4): --deep aggregates ONE tier-2 gate across all bands ─────────────
C15_all_bands_deep_single_gate() {
    make_fixture c15
    local out; run_sweep --all-bands --deep --band-size 2; out="$OUT"
    [ "$RC" -eq 0 ] \
        && pass "C15 --all-bands --deep: exit 0" \
        || fail "C15 --all-bands --deep: exit=$RC (want 0)"
    local g; g="$(echo "$out" | grep -c '<<<TIER2-GATE-SI3' || true)"
    [ "${g:-0}" -eq 1 ] \
        && pass "C15 --all-bands --deep: exactly one TIER2-GATE-SI3 block" \
        || fail "C15 --all-bands --deep: found ${g:-0} gate blocks (want 1), out=$out"
    # Gate rows are '<number><TAB>...': #101 (band0) and #104 (band1) must appear.
    if echo "$out" | grep -q '^101' && echo "$out" | grep -q '^104'; then
        pass "C15 --all-bands --deep: aggregated gate lists #101 (band0) and #104 (band1)"
    else
        fail "C15 --all-bands --deep: gate missing cross-band candidates, out=$out"
    fi
}
...
C16_all_bands_ci_contract() {
    make_fixture c16
    local out; run_sweep --all-bands --band-size 2 --ci-mode; out="$OUT"
    echo "$out" | grep -q '"band_index":"all"' \
        && pass "C16 --all-bands --ci-mode: band_index is the string all" \
        || fail "C16 --all-bands --ci-mode: no band_index=all in JSON, out=$out"
    echo "$out" | grep -q '"bands_swept":2' \
        && pass "C16 --all-bands --ci-mode: bands_swept=2" \
        || fail "C16 --all-bands --ci-mode: bands_swept!=2, out=$out"
    echo "$out" | grep -q '"total_bands":2' \
        && pass "C16 --all-bands --ci-mode: total_bands=2" \
        || fail "C16 --all-bands --ci-mode: total_bands!=2, out=$out"
}

# ── C17 (B6): --max-bands caps the sweep and warns ───────────────────────────
C17_max_bands_upper_bound() {
    make_fixture c17
    local out; run_sweep --all-bands --max-bands 1 --band-size 2 --ci-mode; out="$OUT"
...
# ── C19 (B8): --all-bands --deep --dry-run calls no write helpers ─────────────
C19_all_bands_deep_dry_run() {
    make_fixture c19
    local out; run_sweep --all-bands --deep --dry-run --band-size 2; out="$OUT"
    [ "$RC" -eq 0 ] \
        && pass "C19 --all-bands --deep --dry-run: exit 0" \
        || fail "C19 --all-bands --deep --dry-run: exit=$RC (want 0), out=$out"
    record_has "close-completed" \
        && fail "C19 --all-bands --deep --dry-run: close-completed was called (must not write under --dry-run)" \
        || pass "C19 --all-bands --deep --dry-run: close-completed not called"
    record_has "close-not-planned" \
        && fail "C19 --all-bands --deep --dry-run: close-not-planned was called (must not write under --dry-run)" \
        || pass "C19 --all-bands --deep --dry-run: close-not-planned not called"
}
...
C20_verify_decisions_skip_band_fetch() {
    ...
    make_fixture c20vc
    run_sweep --all-bands --deep --verify-candidates "$tsv"
    ...
    make_fixture c20dec
    run_sweep --all-bands --deep --decisions "$dec"
