...
#
# cmd.exe positives (`rmdir /s`, `rd /s`, `del /s` reached through `cmd /c`) plus
# the false-positive shapes clause-splitting/tokenizing were designed for: the
# verb as an echo argument, a `/s` in a different clause, `/s` as a path tail.
# Text-only: no real cmd.exe process runs here to confirm its own %VAR%/!VAR!
# and escaping semantics match what these payloads assume (TL3 gap, C10 — see
# the dispatcher's `# TL3 gap` block).
...
run_cmdexe_cases() {
    ...
    # round-4: the /c or /k token match is a PREFIX match, not exact. The
    # no-space token "/crd /s dir" starts with "/c"; stripping that 2-char
    # prefix leaves "rd /s dir" as the start of innerText, so this still
    # resolves and blocks (reverts the round-6 approve regression).
    ...
    # detail.md Step 3 + Out of scope explicitly exclude tracking cmd.exe's OWN
    # escape/quote processing (the ^ character). Pinned as the CURRENT
    # (accepted-gap) verdict, not a required one — recorded, not silently
    # dropped.
    ...
    # round-4: a flag-position token (`/`-prefixed) containing % or ! that is
    # not an exact /s match folds the clause to null -> block (fail-closed).
    # These three bare tokens are NOT /-prefixed (verb or target position),
    # so the round-4 rule does not apply to them; they stay approved.
    ...
    expect_approve_cmd "cmd /c rd dir !F! (bare !VAR! at target position, not flag-prefixed)" \
        "cmd /c rd dir !F!"
    # These ARE /-prefixed flag-position tokens containing %/! -> null -> block.
    ...
    expect_block_cmd "cmd /c rd dir /!F! (/-prefixed !VAR! flag token, fails closed, round-4)" \
        "cmd /c rd dir /!F!"
    # Target-position %VAR% paths are unaffected by the round-4 rule either way.
    ...
    # The delete verb is an ARGUMENT of echo, not the head of its clause.
    expect_approve_cmd "cmd /c \"echo rmdir /s test\" (mention through echo)" \
        'cmd /c "echo rmdir /s test"'
    # The /s belongs to the xcopy clause; the rd clause has none, so rd is
    # non-recursive and can only remove an empty directory.
    expect_approve_cmd "cmd /c \"rd dir & xcopy /s foo\" (/s in another clause)" \
        'cmd /c "rd dir & xcopy /s foo"'
    # Round-2 C4: `C:/s` is a path, not an independent /s flag.
    ...
    # cmd.exe's OWN /s switch (before /c) modifies quote-stripping, not the
    # delete verb's recursion — the /c payload here has no /s at all (C8).
    ...
    # SKIPPED: bare `rd`/`rmdir`/`del` + `/s` with no `cmd /c` head
    # Because: `rd`/`del` are cmd.exe internal commands, not standalone
    # executables reachable from git-bash — a bare invocation fails with
    # "command not found" outside a cmd.exe wrapper. `rd` also matches
    # PWSH_RECURSIVE_DELETE_CMDLETS, but hasRecursivePwshFlag only inspects
    # `-`-leading tokens, and real PowerShell has no `/s` switch (only
    # `-Recurse`) — so even under a pwsh alias this is not a working
    # recursive-delete invocation. `cmd /c rd /s dir` (the reachable form)
    # is covered by Step 3. See detail.md Risks & edge cases.
    ...
    # stripClausePrefixes (cmd-exe.js) strips a leading `@`, `if ... (`, or
    # `else` token before candidate-head detection so the verb underneath is
    # still recognized (fixes a round-6 false-negative).
    ...
    # CMD_LAUNCHER_VERBS (cmd-exe.js) treats `call` and `start` as transparent
    # launchers whose remaining argv is re-examined as its own clause
    # candidate.
    ...
    # scanCmdExeText recurses up to MAX_CMD_NEST levels deep so a delete
    # buried inside an inner cmd /c still resolves.
    ...
    # cmdVerbBasename (cmd-exe.js) strips a directory prefix and .exe suffix
    # before matching the verb, so a fully-qualified path to the internal
    # command's external counterpart still resolves.
    ...
    # foreignClauseBlocks (cmd-exe.js) hands a candidate clause whose head is
    # actually a pwsh/powershell invocation back to the pwsh judge instead of
    # treating it as an unrecognized cmd.exe verb.
    ...
    # A pathological input that exceeds MAX_CLAUSE_CANDIDATES / MAX_CMD_NEST
    # must fold to block (fail-closed) rather than silently giving up and
    # approving once the scanner's own bookkeeping limit is hit.
    # MAX_CLAUSE_CANDIDATES only bounds the call/start launcher fan-out
    # (clauseCandidates), not the outer &-clause count — so the overflow guard
    # is exercised by a `start`-launched clause with far more non-flag words
    # than the 32-candidate budget, each of which becomes its own fan-out
    # candidate.
    ...
    # round9: hasDynamicHead now checks ONLY delayed-expansion !VAR! at the
    # clause HEAD, because %VAR% there is substituted once at parse time
    # (before any same-line `set` can affect it) while !VAR! resolves
    # per-command under `setlocal enabledelayedexpansion`/`cmd /v:on` and CAN
    # pick up a same-line assignment — so only !VAR! at the head must fail
    # closed; a bare %VAR% head is now safely approved when the rest of the
    # clause is genuinely harmless.
    ...
    # round9 C11 finding (bin/mutation-probe.sh --test-cmd against this suite):
    # cmd-exe.js's DYNAMIC_VAR_RE (matching BOTH %VAR% and !VAR!) is defined but
    # never referenced anywhere in the file — dead code left behind by the
    # round9 clauseVerdict rewrite above (DELAYED_VAR_RE, !VAR!-only, is what
    # hasDynamicHead actually uses). No test can kill this mutation because the
    # constant is unreachable; reported as a source finding, out of scope here.
