...
# ---------------------------------------------------------------------------
# T12: writeWaiting adds ⏳ prefix to current title
# ---------------------------------------------------------------------------
run_t12() {
  local sid="t12-session-ww"
  local tmp_bash="$TMPDIR_BASE/t12"
  local tr_base_bash="$tmp_bash/transcript"
  local tmp_node; tmp_node=$(to_node_path "$tmp_bash")
  local tr_base_node; tr_base_node=$(to_node_path "$tr_base_bash")
  local tdir_bash; tdir_bash=$(make_transcript_dir "$tr_base_bash" "$tmp_node")
  local jsonl_bash="$tdir_bash/${sid}.jsonl"
  local jsonl_node; jsonl_node=$(to_node_path "$jsonl_bash")

  make_jsonl_with_title "$jsonl_bash" "$sid" "#299 my-issue"
  call_lib_fn "$tr_base_node" "m.writeWaiting('$sid', '$tmp_node');"

  local title; title=$(read_last_title "$jsonl_node" "$sid")
  if [ "$title" = "⏳ #299 my-issue" ]; then
    pass "T12: writeWaiting prepends ⏳ to existing title"
  else
    fail "T12: writeWaiting expected '⏳ #299 my-issue', got '$title'"
  fi
}

# ---------------------------------------------------------------------------
# T13: writeWaiting is idempotent (second call is no-op)
# ---------------------------------------------------------------------------
run_t13() {
  local sid="t13-session-ww"
  ...
  local title; title=$(read_last_title "$jsonl_node" "$sid")
  if [ "$title" = "⏳ #299 my-issue" ]; then
    pass "T13: writeWaiting is idempotent (no double ⏳)"
  else
    fail "T13: writeWaiting idempotent expected '⏳ #299 my-issue', got '$title'"
  fi
}

# ---------------------------------------------------------------------------
# T14: writeWaiting skips when title starts with ✓ (session complete)
# ---------------------------------------------------------------------------
run_t14() {
  local sid="t14-session-ww"
  ...
  make_jsonl_with_title "$jsonl_bash" "$sid" "✓ #299 my-issue PR #1086"
  call_lib_fn "$tr_base_node" "m.writeWaiting('$sid', '$tmp_node');"

  local title; title=$(read_last_title "$jsonl_node" "$sid")
  if [ "$title" = "✓ #299 my-issue PR #1086" ]; then
    pass "T14: writeWaiting skips when title starts with ✓"
  else
    fail "T14: writeWaiting should skip for ✓ title, got '$title'"
  fi
}

# ---------------------------------------------------------------------------
# T15: writeClearWaiting removes ⏳ prefix
# ---------------------------------------------------------------------------
run_t15() {
  local sid="t15-session-ww"
  ...
  make_jsonl_with_title "$jsonl_bash" "$sid" "⏳ #299 my-issue PR #1086"
  call_lib_fn "$tr_base_node" "m.writeClearWaiting('$sid', '$tmp_node');"

  local title; title=$(read_last_title "$jsonl_node" "$sid")
  if [ "$title" = "#299 my-issue PR #1086" ]; then
    pass "T15: writeClearWaiting removes ⏳ prefix"
  else
    fail "T15: writeClearWaiting expected '#299 my-issue PR #1086', got '$title'"
  fi
}

# ---------------------------------------------------------------------------
# T16: writeClearWaiting is no-op when no ⏳ prefix
# ---------------------------------------------------------------------------
run_t16() {
  local sid="t16-session-ww"
  ...
  local count; count=$(count_title_records "$jsonl_node" "$sid")
  if [ "$count" = "1" ]; then
    pass "T16: writeClearWaiting is no-op when no ⏳ (record count unchanged)"
  else
    fail "T16: writeClearWaiting no-op expected 1 record, got $count"
  fi
}

# ---------------------------------------------------------------------------
# T17: writeMarkComplete strips ⏳ before adding ✓
# ---------------------------------------------------------------------------
run_t17() {
  local sid="t17-session-ww"
  ...
  make_jsonl_with_title "$jsonl_bash" "$sid" "⏳ #299 my-issue"
  call_lib_fn "$tr_base_node" "m.writeMarkComplete('$sid', '$tmp_node');"

  local title; title=$(read_last_title "$jsonl_node" "$sid")
  if [ "$title" = "✓ #299 my-issue" ]; then
    pass "T17: writeMarkComplete strips ⏳ before adding ✓"
  else
    fail "T17: writeMarkComplete expected '✓ #299 my-issue', got '$title'"
  fi
}
