...
# Tags: workflow, gate, hook, bin, windows
...
const require = createRequire(import.meta.url);
const { parseGitCArg } = require(process.env.GIT_CMD_PATH);
const r = parseGitCArg('git -C "c:/LLM/my-specs-repo" commit');
process.stdout.write(r === null ? 'null' : r);
EOF
  )
  assert_eq 'D2: double-quoted path' "c:/LLM/my-specs-repo" "$result"
...
EOF
  ...
  assert_eq "D3: single-quoted path" "c:/LLM/my-specs-repo" "$result"
...
const require = createRequire(import.meta.url);
const { resolveRepoDir } = require(process.env.HOOK_PATH);
process.stdout.write(resolveRepoDir('git -C "c:/LLM/my-specs-repo" commit'));
EOF
  )
  if [ "$(uname -s | tr '[:upper:]' '[:lower:]')" = "windows_nt" ] || echo "$OS" | grep -qi windows 2>/dev/null; then
    expected_eq='c:\LLM\my-specs-repo'
  else
...
const r = extractRepoDirFromCommand('git -C "c:/LLM/my-specs-repo" commit');
process.stdout.write(r === null ? 'null' : r);
EOF
  )
  assert_eq 'G1: double-quoted -> c:/LLM/my-specs-repo' "c:/LLM/my-specs-repo" "$result"
