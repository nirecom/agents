...
run_c3_index_mode() {
  ...
  mode="$(git -C "$AGENTS_DIR" ls-files -s -- bin/vscode-patch-include-worktrees \
    | awk '{print $1}')"
  if [ -z "$mode" ]; then
    echo "SKIP: C3-e02 git index mode (bin/vscode-patch-include-worktrees not tracked yet)"
    SKIP=$((SKIP + 1))
    return 0
  fi
...
run_c4_readme_entry() {
  ...
  if [ ! -f "$readme" ]; then
    echo "FAIL: C4-r02: README.md registers bin/vscode-patch-include-worktrees -- no README.md"
    FAIL=$((FAIL + 1))
    return 0
  fi
  body="$(cat "$readme")"
  check_contains "C4-r02: README.md registers bin/vscode-patch-include-worktrees" \
    "bin/vscode-patch-include-worktrees" "$body"
}
