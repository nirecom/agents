...
run_g2_regression() {
    ...
    local tmp out rc
    tmp="$(mktemp -d)"
    seed_state "$tmp" "g2r-sid" "{ l2_armed_at: null, last_run_at: null, cumulative_severity: 'error', findings: [{\"categories\":[\"code\"],\"severity\":\"error\",\"detail\":\"d\",\"timestamp\":\"2026-06-06T12:00:00.000Z\"}] }"
...
run_g19_regression() {
    ...
    local tmp out rc
    tmp="$(mktemp -d)"
    seed_state "$tmp" "g19r-sid" "{ l2_armed_at: null, last_run_at: null, cumulative_severity: 'error', findings: [{\"categories\":[\"workflow\"],\"severity\":\"error\",\"detail\":\"first-finding\",\"timestamp\":\"2026-06-06T11:00:00.000Z\"},{\"categories\":[\"workflow\"],\"severity\":\"error\",\"detail\":\"last-finding\",\"timestamp\":\"2026-06-06T12:00:00.000Z\"}] }"
...
run_g20_regression() {
    ...
    local tmp out rc
    tmp="$(mktemp -d)"
    seed_state "$tmp" "g20r-sid" "{ l2_armed_at: null, last_run_at: null, cumulative_severity: 'error', findings: [{\"categories\":[\"code\"],\"severity\":\"error\",\"detail\":\"d\",\"timestamp\":\"2026-06-06T12:00:00.000Z\"}] }"
...
run_g21_regression() {
    ...
    seed_state "$tmp" "g21r-sid" "{ l2_armed_at: null, last_run_at: null, cumulative_severity: 'error', findings: [{\"categories\":[\"code\"],\"severity\":\"error\",\"detail\":\"d\",\"timestamp\":\"2026-06-06T12:00:00.000Z\"}] }"
...
run_g22_regression() {
    ...
    wsid="${TODAY}-g22rwsid"
    printf "Session-ID: %s\n" "$wsid" > "$tmp/WORKTREE_NOTES.md"
    seed_state "$tmp" "g22r-sid" "{ l2_armed_at: null, last_run_at: null, cumulative_severity: 'error', findings: [{\"categories\":[\"code\"],\"severity\":\"error\",\"detail\":\"d\",\"timestamp\":\"2026-06-06T12:00:00.000Z\"}] }"
...
run_gn1() {
    ...
    local tmp out rc
    tmp="$(mktemp -d)"
    seed_state "$tmp" "gn1-sid" "{ l2_armed_at: null, last_run_at: null, cumulative_severity: 'error', findings: [{\"categories\":[\"workflow\",\"code\"],\"severity\":\"error\",\"detail\":\"d\",\"timestamp\":\"2026-06-06T12:00:00.000Z\"}] }"
...
run_gn2() {
    ...
    local tmp out rc
    tmp="$(mktemp -d)"
    seed_state "$tmp" "gn2-sid" "{ l2_armed_at: '2026-06-06T12:00:00Z', last_run_at: null, cumulative_severity: null, findings: [] }"
...
run_b4_warning() {
    ...
    local tmp out rc
    tmp="$(mktemp -d)"
    seed_state "$tmp" "b4w-sid" "{ l2_armed_at: null, last_run_at: null, cumulative_severity: 'warning', findings: [{\"categories\":[\"code\"],\"severity\":\"warning\",\"detail\":\"advisory\",\"timestamp\":\"2026-06-06T12:00:00.000Z\"}] }"
...
run_b4_notice() {
    ...
    local tmp out rc
    tmp="$(mktemp -d)"
    seed_state "$tmp" "b4n-sid" "{ l2_armed_at: null, last_run_at: null, cumulative_severity: 'notice', findings: [{\"categories\":[\"code\"],\"severity\":\"notice\",\"detail\":\"advisory\",\"timestamp\":\"2026-06-06T12:00:00.000Z\"}] }"
...
run_b5_null_state() {
    ...
    local tmp out rc
    tmp="$(mktemp -d)"
    seed_state "$tmp" "b5-sid" "{ l2_armed_at: null, last_run_at: null, cumulative_severity: null, findings: [] }"
...
run_b_l2phase_done() {
    require_source "$HOOK" "B-l2phase-done: l2ArmedAt set but l2_phase=done exits 0" || return
    local tmp out rc
    tmp="$(mktemp -d)"
    seed_state "$tmp" "l2done-sid" "{ l2_armed_at: '2026-06-06T12:00:00Z', l2_phase: 'done', last_run_at: null, cumulative_severity: null, findings: [] }"
    out=$(echo '{"stop_hook_active":false,"session_id":"l2done-sid","transcript_path":""}' \
        | WORKFLOW_PLANS_DIR="$tmp" run_with_timeout 5 node "$HOOK" 2>/dev/null)
    rc=$?
    rm -rf "$tmp"
    if [ $rc -eq 0 ] && ! echo "$out" | grep -q '"decision":"block"'; then
        pass "B-l2phase-done: l2ArmedAt set but l2_phase=done exits 0 (no block)"
    else
        fail "B-l2phase-done: l2ArmedAt set but l2_phase=done exits 0 (rc=$rc, out=$out)"
    fi
}

run_b_l2phase_frozen() {
    require_source "$HOOK" "B-l2phase-frozen: l2ArmedAt set but l2_phase=frozen exits 0" || return
    local tmp out rc
    tmp="$(mktemp -d)"
    seed_state "$tmp" "l2frozen-sid" "{ l2_armed_at: '2026-06-06T12:00:00Z', l2_phase: 'frozen', last_run_at: null, cumulative_severity: null, findings: [] }"
    out=$(echo '{"stop_hook_active":false,"session_id":"l2frozen-sid","transcript_path":""}' \
        | WORKFLOW_PLANS_DIR="$tmp" run_with_timeout 5 node "$HOOK" 2>/dev/null)
    rc=$?
    rm -rf "$tmp"
    if [ $rc -eq 0 ] && ! echo "$out" | grep -q '"decision":"block"'; then
        pass "B-l2phase-frozen: l2ArmedAt set but l2_phase=frozen exits 0 (no block)"
    else
        fail "B-l2phase-frozen: l2ArmedAt set but l2_phase=frozen exits 0 (rc=$rc, out=$out)"
    fi
}
...
run_b_workflow_off() {
    ...
    seed_state "$tmp" "woff-sid" "{ l2_armed_at: null, last_run_at: null, cumulative_severity: 'error', findings: [{\"categories\":[\"code\"],\"severity\":\"error\",\"detail\":\"d\",\"timestamp\":\"2026-06-06T12:00:00.000Z\"}] }"
