...
# lines, capped at 81 after this change; #2140 grew it to 84). This pins: both scripts exist and
...
echo ""
echo "--- C: SKILL.md stays within the size cap (<=84 lines; Pattern B WARN=100/HARD=200) ---"

if [ -f "$SKILL_MD" ]; then
  LINES=$(wc -l < "$SKILL_MD")
  if [ "$LINES" -le 84 ]; then
    pass "C1 SKILL.md is $LINES lines (<=84)"
  else
    fail "C1 SKILL.md is $LINES lines, exceeds the 84-line cap"
  fi
