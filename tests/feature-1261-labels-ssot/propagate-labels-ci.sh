...
grep "git clone" "$MOCK_LOG" 2>/dev/null | grep -q "nirecom/my-private-repo" && DOTFILES_PRIVATE_CLONED=1
...
if [ "$DOTFILES_CLONED" = "1" ] && [ "$DOTFILES_PRIVATE_CLONED" = "1" ]; then
    pass "T-propagate-multi: both siblings cloned (dotfiles + my-private-repo)"
else
...
case "$1" in
  clone)
    DEST="${!#}"
    # Fail if cloning dotfiles (first sibling), succeed for my-private-repo
    if echo "$DEST" | grep -q "my-private-repo"; then
        mkdir -p "$DEST/.github"
...
SECOND_ATTEMPTED=0
grep "git clone" "$MOCK_LOG" 2>/dev/null | grep -q "my-private-repo" && SECOND_ATTEMPTED=1
