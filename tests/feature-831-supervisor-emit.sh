...
# --- E4: reportFallback ---
run_e4() {
    require_source "$EMIT_MODULE" "E4: reportFallback emits workflow/warning" || return
    local prog out
...
console.log('OK');
" "$out" 2>&1)
    [ "$check" = "OK" ] && pass "E4: reportFallback emits workflow/warning" \
        || fail "E4: reportFallback emits workflow/warning ($check)"
}
