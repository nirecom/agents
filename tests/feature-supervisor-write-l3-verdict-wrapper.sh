#!/bin/bash
# tests/feature-supervisor-write-audit-verdict-wrapper.sh
# Tests: bin/supervisor-write-audit-verdict
...
CLI="$AGENTS_DIR/bin/supervisor-write-audit-verdict"
CLI_NODE="$_AGENTS_DIR_NODE/bin/supervisor-write-audit-verdict"
...
process.stdout.write(String(st.audit.$field));
" 2>/dev/null
}

# Skip-gate: wrapper not yet created
if [ ! -f "$CLI" ]; then
    skip "V1: valid CONTINUE -> audit_phase=done, audit_verdict=CONTINUE, audit_cause set, exit 0 (wrapper not yet created)"
    skip "V2: valid WARN -> audit_verdict=WARN (wrapper not yet created)"
    skip "V3: valid BLOCK -> audit_verdict=BLOCK (wrapper not yet created)"
...
# V2 — valid WARN: audit_verdict=WARN.
run_v2() {
    local label="V2: valid WARN -> audit_verdict=WARN"
    ...
    rm -rf "$tmp"
    if [ $rc -eq 0 ] && [ "$audit_verdict" = "WARN" ]; then
        pass "$label"
    else
        fail "$label (rc=$rc, audit_verdict=$audit_verdict)"
    fi
}

# V3 — valid BLOCK: audit_verdict=BLOCK.
run_v3() {
    local label="V3: valid BLOCK -> audit_verdict=BLOCK"
    local tmp rc audit_verdict
    tmp="$(mktemp -d)"
    unset WORKFLOW_SESSION_ID || true
    unset CLAUDE_SESSION_ID || true
    ...
    rm -rf "$tmp"
    if [ $rc -eq 0 ] && [ "$audit_verdict" = "BLOCK" ]; then
        pass "$label"
    else
        fail "$label (rc=$rc, audit_verdict=$audit_verdict)"
    fi
