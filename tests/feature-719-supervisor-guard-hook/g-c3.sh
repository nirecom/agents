...
try {
  ...
  process.stdout.write(String((s.layer2 || {}).l2_cause || ''));
} catch (e) {
...
try {
  const s = JSON.parse(fs.readFileSync(p, 'utf8'));
  process.stdout.write(String((s.layer2 || {}).l2_phase || ''));
} catch (e) {
  process.stdout.write('');
}
" 2>/dev/null
...
run_g_c3a() {
    ...
    seed_state "$tmp" "g-c3a-sid" "{ l2_armed_at: null, last_run_at: null, cumulative_severity: null, findings: [] }"
    ...
    cause=$(WORKFLOW_PLANS_DIR="$tmp" read_l2_cause "$tmp" "g-c3a-sid")
    rm -rf "$tmp"
...
run_g_c3b() {
    ...
    tp="$(node_path "$tmp/t.jsonl")"
    seed_state "$tmp" "g-c3b-sid" "{ l2_armed_at: null, last_run_at: null, cumulative_severity: null, findings: [] }"
    printf '{"stop_hook_active":false,"session_id":"g-c3b-sid","transcript_path":"%s"}' "$tp" \
        | WORKFLOW_PLANS_DIR="$tmp" run_with_timeout 5 node "$HOOK" >/dev/null 2>&1
    cause=$(WORKFLOW_PLANS_DIR="$tmp" read_l2_cause "$tmp" "g-c3b-sid")
    rm -rf "$tmp"
...
run_g_c3c() {
    ...
    make_fixture "$tmp/t.jsonl" \
        ...
        '{"type":"assistant","message":{"role":"assistant","content":[{"type":"text","text":"I will use <<WORKFLOW_ENFORCE_WORKTREE_OFF: some reason>>"}]}}'
    tp="$(node_path "$tmp/t.jsonl")"
    seed_state "$tmp" "g-c3c-sid" "{ l2_armed_at: null, last_run_at: null, cumulative_severity: null, findings: [] }"
    printf '{"stop_hook_active":false,"session_id":"g-c3c-sid","transcript_path":"%s"}' "$tp" \
        | WORKFLOW_PLANS_DIR="$tmp" run_with_timeout 5 node "$HOOK" >/dev/null 2>&1
    phase=$(WORKFLOW_PLANS_DIR="$tmp" read_l2_phase "$tmp" "g-c3c-sid")
    rm -rf "$tmp"
