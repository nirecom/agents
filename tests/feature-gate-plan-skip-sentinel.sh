...
# ---------------------------------------------------------------------------
# T17–T20: Recorded-verdict skip (#1286)
# These cases test the new hasValidSkipJudgment path in gate-plan-skip-sentinel.js.
# Expected RED until #1286 write-code implements:
#   - recordSkipJudgment / hasValidSkipJudgment in skip-signal-resolver.js
#   - gate-plan-skip-sentinel.js reads hasValidSkipJudgment(sid, "outline"|"detail")
# ---------------------------------------------------------------------------

SKIP_RESOLVER="$AGENTS_DIR/hooks/lib/workflow-state/skip-signal-resolver.js"

if [ ! -f "$SKIP_RESOLVER" ] || ! node -e "
  const r = require('$(cygpath -m "$SKIP_RESOLVER" 2>/dev/null || echo "$SKIP_RESOLVER")');
  if (typeof r.recordSkipJudgment !== 'function') process.exit(1);
" 2>/dev/null; then
  echo "SKIP: T17-T20: skip-judgment gate path (recordSkipJudgment not yet implemented)"
  echo ""
  echo "=== Results ==="
  if [ "$ERRORS" -eq 0 ]; then echo "All tests passed!"; else echo "$ERRORS test(s) failed"; exit 1; fi
  exit 0
fi
...
GATE_WF_DIR="$TMPDIR_GATE/workflow-state"
mkdir -p "$GATE_WF_DIR"
GATE_WF_DIR_W="$(cygpath -m "$GATE_WF_DIR" 2>/dev/null || echo "$GATE_WF_DIR")"

# Plant a valid outline skip_judgment for T17
node -e "
  process.env.CLAUDE_WORKFLOW_DIR='$GATE_WF_DIR_W';
  const r = require('$RESOLVER_N');
  r.recordSkipJudgment('t17', 'outline', { so_c1: true, so_c2: true }, 'orchestrator');
" 2>/dev/null || true

echo "=== T17: OUTLINE_NOT_NEEDED + valid skip_judgment → allow (no CONFIRM_OUTLINE env) ==="
INPUT_T17=$(build_bash_input 'echo "<<WORKFLOW_OUTLINE_NOT_NEEDED: recorded judgment>>"')
OUT_T17="$(unset CONFIRM_OUTLINE 2>/dev/null || true
  CLAUDE_WORKFLOW_DIR="$GATE_WF_DIR" WORKFLOW_SESSION_ID="t17" run_gate "$INPUT_T17")"
if echo "$OUT_T17" | grep -q '"permissionDecision":"allow"'; then
    pass "T17. OUTLINE_NOT_NEEDED + valid skip_judgment → allow"
else
    fail "T17. expected allow, got: $OUT_T17"
fi

echo "=== T18: OUTLINE_NOT_NEEDED + no record + CONFIRM_OUTLINE unset → pass-through ==="
INPUT_T18=$(build_bash_input 'echo "<<WORKFLOW_OUTLINE_NOT_NEEDED: no record>>"')
_T18_ENV=$(mktemp -d)
OUT_T18="$(unset CONFIRM_OUTLINE 2>/dev/null || true
  CLAUDE_WORKFLOW_DIR="$_T18_ENV" WORKFLOW_SESSION_ID="t18-no-record" run_gate "$INPUT_T18")"
if [ -z "$OUT_T18" ] || [ "$OUT_T18" = "{}" ] || ! echo "$OUT_T18" | grep -q '"permissionDecision"'; then
    pass "T18. OUTLINE_NOT_NEEDED + no record + CONFIRM_OUTLINE unset → pass-through"
else
    fail "T18. expected pass-through, got: $OUT_T18"
fi
rmdir "$_T18_ENV" 2>/dev/null || true

# Plant a valid detail skip_judgment for T19
node -e "
  process.env.CLAUDE_WORKFLOW_DIR='$GATE_WF_DIR_W';
  const r = require('$RESOLVER_N');
  r.recordSkipJudgment('t19', 'detail', { sd_c1: true, sd_c2: true, sd_c3: true }, 'orchestrator');
" 2>/dev/null || true

echo "=== T19: DETAIL_NOT_NEEDED + valid skip_judgment → allow (no CONFIRM_DETAIL env) ==="
INPUT_T19=$(build_bash_input 'echo "<<WORKFLOW_DETAIL_NOT_NEEDED: recorded judgment>>"')
_T19_ENV=$(mktemp -d)
OUT_T19="$(unset CONFIRM_DETAIL 2>/dev/null || true
  CLAUDE_WORKFLOW_DIR="$GATE_WF_DIR" WORKFLOW_SESSION_ID="t19" AGENTS_CONFIG_DIR="$_T19_ENV" run_gate "$INPUT_T19")"
if echo "$OUT_T19" | grep -q '"permissionDecision":"allow"'; then
    pass "T19. DETAIL_NOT_NEEDED + valid skip_judgment → allow"
else
    fail "T19. expected allow, got: $OUT_T19"
fi
rmdir "$_T19_ENV" 2>/dev/null || true

echo "=== T20: DETAIL_NOT_NEEDED + no record + CONFIRM_DETAIL unset → pass-through ==="
INPUT_T20=$(build_bash_input 'echo "<<WORKFLOW_DETAIL_NOT_NEEDED: no record>>"')
_T20_ENV=$(mktemp -d)
OUT_T20="$(unset CONFIRM_DETAIL 2>/dev/null || true
  CLAUDE_WORKFLOW_DIR="$_T20_ENV" WORKFLOW_SESSION_ID="t20-no-record" AGENTS_CONFIG_DIR="$_T20_ENV" run_gate "$INPUT_T20")"
if [ -z "$OUT_T20" ] || [ "$OUT_T20" = "{}" ] || ! echo "$OUT_T20" | grep -q '"permissionDecision"'; then
    pass "T20. DETAIL_NOT_NEEDED + no record + CONFIRM_DETAIL unset → pass-through"
else
    fail "T20. expected pass-through, got: $OUT_T20"
fi
rmdir "$_T20_ENV" 2>/dev/null || true
