...
EXCL="tests/bin/fix-992-bare-python.sh"                 # an EXCLUDED_FILES entry
EXCL_CTRL="tests/hooks/feature-enforce-system-ops.sh"     # a different entry, mutation control
...
sed 's|^  "tests/bin/fix-992-bare-python.sh"|  "tests/zzz-not-a-real-entry.sh"|' "$SCRIPT" > "$MUTANT"
