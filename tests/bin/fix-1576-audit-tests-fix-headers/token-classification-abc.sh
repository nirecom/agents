...
# Sourced fragment — TC1-TC10: A/B/C token classification (#1576).
# NOT a standalone test; depends on caller for $AUDIT, $AUDIT_COMMON,
# PASS/FAIL, pass(), fail(), make_fixture(), write_dispatcher(), run_in().
...
before="$(cat "$R1/tests/bin/feature-1-a.sh")"
run_in "$R1" "$AUDIT" --dry-run --fix-headers --offline
after="$(cat "$R1/tests/bin/feature-1-a.sh")"
...
new_hdr="$(grep -m1 -E '^# Tests:' "$R2/tests/bin/feature-2-a.sh" || true)"
is_exec=0; [[ -x "$R2/tests/bin/feature-2-a.sh" ]] && is_exec=1
...
before="$(cat "$R4/tests/bin/feature-4-ac.sh")"
run_in "$R4" "$AUDIT" --fix-headers --apply --offline
after="$(cat "$R4/tests/bin/feature-4-ac.sh")"
...
before="$(cat "$R7/tests/bin/feature-7-multiparen.sh")"
run_in "$R7" "$AUDIT" --fix-headers --apply --offline
after="$(cat "$R7/tests/bin/feature-7-multiparen.sh")"
...
if [[ ! -f "$AUDIT_COMMON" ]]; then
  ...
  before="$(cat "$R8/tests/bin/check-something.sh")"
  run_in "$R8" "$AUDIT_COMMON" --fix-headers
  after="$(cat "$R8/tests/bin/check-something.sh")"
  if [[ "$OUT$ERR" == *"FIX_A:"* && "$before" == "$after" ]]; then
