...
# #2392: the scan globs are tests/<category>/, so fixtures live under tests/bin/.
...
add_raw "$L_REPO" "bin/feature-820-x.Tests.ps1" <<'EOF'
...
add_raw "$L_REPO" "bin/cc-l.Tests.ps1" <<'EOF'
...
add_raw "$L_REPO" "bin/test_l_guard.py" <<'EOF'
# Tests: bin/l-dead.py
# Tags: TL2, scope:common
def test_x():
    assert True
...
add_raw "$L_REPO" "bin/cc-l-alive.Tests.ps1" <<'EOF'
...
L_IS_OUT="$OUT"
if line_has "$L_IS_OUT" CANDIDATE "tests/bin/feature-820-x.Tests.ps1"; then
...
if line_has "$L_IS_OUT" CANDIDATE "tests/bin/cc-l.Tests.ps1"; then
...
if line_has "$L_CC_OUT" ORPHAN "tests/bin/cc-l-alive.Tests.ps1"; then
