#!/usr/bin/env bash
# tests/bin/fix-1532-node-guard-get-config-var.sh
...
# filename stem, so this filename is what makes an edit to bin/get-config-var select
# a suite that actually checks it. All logic lives in fix-1532-node-guard/.
