...
#
# TL2 test of how bin/audit-tests.sh uses closed_at (revised for #1833).
# closed_at gates deletion only; candidacy = target survival (every # Tests:
# token format-OK, missing, not rename-resolvable). TL3 gap: real gh API
# transport; mitigation: manual online run before merge.
...
EOF
  {
    ...
    echo 'echo foo'
  } > "$root/tests/bin/feature-1557-foo.sh"
...
DISPATCHER="tests/bin/feature-1557-foo.sh"
