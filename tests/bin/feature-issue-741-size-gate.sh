...
# Tags: skill-size, code-size, review, bin, hard-block, prompt, scope:common
...
# ---------------------------------------------------------------------------
# Case 13 (#1834/#1788): review-code-size --staged excludes _archive/ paths
# ---------------------------------------------------------------------------
REPO=$(make_repo)
git -C "$REPO" checkout -q -b feat-c13
mkdir -p "$REPO/bin" "$REPO/sub/_archive"
make_lines 350 > "$REPO/bin/normal.js"
make_lines 600 > "$REPO/sub/_archive/old.js"
git -C "$REPO" add -A
EXIT_CODE=0
OUTPUT=$(cd "$REPO" && run_with_timeout bash "$SCRIPT_CODE" --staged 2>&1) || EXIT_CODE=$?
if [[ "$OUTPUT" == *"bin/normal.js"* && "$OUTPUT" != *"_archive/old.js"* && $EXIT_CODE -eq 0 ]]; then
    pass "Case 13: review-code-size --staged excludes _archive/ paths (normal.js kept, archive dropped)"
else
    fail "Case 13: --staged excludes _archive/ paths (rc=$EXIT_CODE)"
fi

# ---------------------------------------------------------------------------
# Case 14 (#1834/#1788): review-code-size diff mode excludes _archive/ files
# ---------------------------------------------------------------------------
REPO=$(make_repo)
git -C "$REPO" checkout -q -b feat-c14
mkdir -p "$REPO/bin" "$REPO/sub/_archive"
make_lines 350 > "$REPO/bin/normal.js"
make_lines 600 > "$REPO/sub/_archive/old.js"
git -C "$REPO" add -A
...
OUTPUT=$(cd "$REPO" && run_with_timeout bash "$SCRIPT_CODE" --base main 2>&1) || EXIT_CODE=$?
if [[ "$OUTPUT" == *"bin/normal.js"* && "$OUTPUT" != *"_archive/old.js"* && $EXIT_CODE -eq 0 ]]; then
    pass "Case 14: review-code-size diff-mode filter_code_targets excludes _archive/ files"
else
    fail "Case 14: diff-mode filter_code_targets excludes _archive/ files (rc=$EXIT_CODE)"
fi

# ---------------------------------------------------------------------------
# Case 15 (#1788): _archived/ parity between diff mode and --staged mode
# SKIP when the two modes still disagree (pre-fix); PASS when consistent.
# ---------------------------------------------------------------------------
REPO=$(make_repo)
git -C "$REPO" checkout -q -b feat-c15-diff
mkdir -p "$REPO/bin" "$REPO/sub/_archived"
make_lines 350 > "$REPO/bin/keep.js"
make_lines 600 > "$REPO/sub/_archived/dead.js"
git -C "$REPO" add -A
git -C "$REPO" commit -q -m "add files"
DIFF_OUT=$(cd "$REPO" && run_with_timeout bash "$SCRIPT_CODE" --base main 2>&1 || true)
diff_has_archived=0
if echo "$DIFF_OUT" | grep -q '_archived/dead.js'; then diff_has_archived=1; fi

REPO=$(make_repo)
git -C "$REPO" checkout -q -b feat-c15-staged
mkdir -p "$REPO/bin" "$REPO/sub/_archived"
make_lines 350 > "$REPO/bin/keep.js"
make_lines 600 > "$REPO/sub/_archived/dead.js"
git -C "$REPO" add -A
STAGED_OUT=$(cd "$REPO" && run_with_timeout bash "$SCRIPT_CODE" --staged 2>&1 || true)
staged_has_archived=0
if echo "$STAGED_OUT" | grep -q '_archived/dead.js'; then staged_has_archived=1; fi

if [[ "$diff_has_archived" -eq "$staged_has_archived" ]]; then
    pass "Case 15: _archived/ treated consistently across diff and --staged modes"
else
    echo "SKIP: Case 15: _archived/ parity not yet applied (diff=$diff_has_archived staged=$staged_has_archived) — #1788 fix pending"
fi
