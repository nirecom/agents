...
#
# TL2 test of bin/audit-tests-common.sh orphan detection (revised for #1833).
# Orphan = common-scope dispatcher with all # Tests: paths missing. Prose
# tokens are MALFORMED_HEADER, not missing paths. Write mode apply-by-default;
# use --dry-run for report-only cases. TL3 gap: interaction with
# audit-tests.sh online emission; mitigation: manual full audit run.
...
if [[ "$OUT" == *"NO_TESTS_HEADER: tests/bin/cc-nohdr.sh"* ]]; then
...
if [[ "$OUT" == *"MALFORMED_HEADER: tests/bin/cc-prose.sh"* ]]; then
...
root="$(new_repo)"
{
  ...
  echo '# Tags: TL2, scope:common'
  echo 'echo hi'
} > "$root/tests/bin/cc-empty-tests-header.sh"
commit "$root"
run_common "$root"
if [[ $RC -eq 1 && "$OUT" != *"ORPHAN:"* ]]; then
...
if [[ "$OUT" == *"NO_TESTS_HEADER: tests/bin/cc-empty-tests-header.sh"* ]]; then
