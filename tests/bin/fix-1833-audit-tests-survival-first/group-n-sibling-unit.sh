...
# modes are specific to it: partial deletion (dispatcher removed, folder orphaned,
# a tree that no longer runs) and metadata drift (last_commit / sibling_file_count
# from the dispatcher alone, so folder-heavy stems look abandoned).
...
n_unit() {
    ...
    mkdir -p "$root/tests/bin/$stem"
    for ((i = 1; i <= count; i++)); do
        {
            printf '# Tests: %s\n' "$hdr"
            printf '# Tags: %s\n' "$tags"
            printf 'echo group-%s\n' "$i"
        } > "$root/tests/bin/$stem/group-$i.sh"
    done
}
...
assert_eq "N1a the unit is reported as one candidate, keyed on the dispatcher" \
    "candidate" "$(report_of "$N1_OUT" "tests/bin/feature-1301-deadunit.sh")"
...
if printf '%s\n' "$N1_OUT" | grep -qE "Sibling folder: tests/bin/feature-1301-deadunit/ \(3 files\)"; then
    pass "N1b the report names the sibling folder with its 3-file count"
else
    fail "N1b expected 'Sibling folder: tests/bin/feature-1301-deadunit/ (3 files)' (out=<<$N1_OUT>>)"
fi

# N1c — and it states the deletion unit explicitly, so the reader knows the
# folder goes with it before authorising anything.
if printf '%s\n' "$N1_OUT" | grep -qE "Deletion unit: tests/bin/feature-1301-deadunit\.sh tests/bin/feature-1301-deadunit/"; then
...
N1D_GOT=""
for n_g in 1 2 3; do
    N1D_GOT="$N1D_GOT$(report_of "$N1_OUT" "tests/bin/feature-1301-deadunit/group-$n_g.sh") "
done
...
assert_eq "N2b the candidate's sibling key carries the folder path" \
    "tests/bin/feature-1301-deadunit/" \
    "$(json_query "$N2_JSON" '((d.candidates||[]).find(c=>c.dispatcher==="tests/bin/feature-1301-deadunit.sh")||{}).sibling')"
assert_eq "N2c sibling_file_count matches the 3 files really in the folder" \
    "3" \
    "$(json_query "$N2_JSON" '((d.candidates||[]).find(c=>c.dispatcher==="tests/bin/feature-1301-deadunit.sh")||{}).sibling_file_count')"
...
assert_gate_row "N3a the issue-specific dispatcher is deleted" \
    "$N3_OUT" "$N_REPO" "tests/bin/feature-1301-deadunit.sh" candidate deleted gone
assert_eq "N3b its sibling folder went with it — no orphaned directory remains" \
    "gone" "$(fs_of "$N_REPO" "tests/bin/feature-1301-deadunit")"
...
assert_gate_row "N3c the common dispatcher is deleted" \
    "$N3C_OUT" "$N_REPO" "tests/bin/cc-deadunit-n.sh" orphan deleted gone
assert_eq "N3d the common sibling folder went with it" \
    "gone" "$(fs_of "$N_REPO" "tests/bin/cc-deadunit-n")"
...
"D  tests/bin/cc-deadunit-n.sh
D  tests/bin/cc-deadunit-n/group-1.sh
D  tests/bin/cc-deadunit-n/group-2.sh
D  tests/bin/feature-1301-deadunit.sh
D  tests/bin/feature-1301-deadunit/group-1.sh
D  tests/bin/feature-1301-deadunit/group-2.sh
D  tests/bin/feature-1301-deadunit/group-3.sh" \
...
assert_eq "N3f the alive unit survives whole" \
    "kept kept" \
    "$(fs_of "$N_REPO" "tests/bin/cc-liveunit-n.sh") $(fs_of "$N_REPO" "tests/bin/cc-liveunit-n")"
