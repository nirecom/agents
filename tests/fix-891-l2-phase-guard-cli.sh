...
# Tests: hooks/supervisor-guard.js, bin/supervisor-write-layer2, hooks/lib/workflow-state.js
...
CLI_NODE="$_AGENTS_DIR_NODE/bin/supervisor-write-layer2"
...
run_g34() {
    ...
    if [ $rc -eq 0 ] && [ "$phase" = "done" ]; then
        pass "G34: CLI --set-l2-phase done -> state has l2_phase done"
    else
        fail "G34: CLI --set-l2-phase done -> state has l2_phase done (rc=$rc, phase=$phase)"
    fi
}

run_g35() {
    require_source "$CLI" "G35: CLI --set-l2-phase invalid -> exit 1" || return
    local tmp rc
    tmp="$(mktemp -d)"
    WORKFLOW_PLANS_DIR="$tmp" run_with_timeout 5 node "$CLI_NODE" --session-id g35-sid --set-l2-phase bogus >/dev/null 2>&1
    rc=$?
    rm -rf "$tmp"
    if [ $rc -eq 1 ]; then
        pass "G35: CLI --set-l2-phase invalid -> exit 1"
    else
        fail "G35: CLI --set-l2-phase invalid -> exit 1 (rc=$rc)"
    fi
}

run_g36() {
    require_source "$CLI" "G36: CLI --set-l2-phase alone -> succeeds" || return
    local tmp rc phase
    tmp="$(mktemp -d)"
    WORKFLOW_PLANS_DIR="$tmp" run_with_timeout 5 node "$CLI_NODE" --session-id g36-sid --set-l2-phase pending >/dev/null 2>&1
    rc=$?
    phase=$(read_l2_phase "$tmp" "g36-sid")
    rm -rf "$tmp"
    if [ $rc -eq 0 ] && [ "$phase" = "pending" ]; then
        pass "G36: CLI --set-l2-phase alone -> succeeds"
    else
        fail "G36: CLI --set-l2-phase alone -> succeeds (rc=$rc, phase=$phase)"
    fi
}
...
run_g39() {
    require_source "$HOOK" "G39: l2_phase=frozen + final_report sentinel as last tool_use -> exit 0" || return
    local tmp out rc tp
    tmp="$(mktemp -d)"
    ...
    rm -rf "$tmp"
    if [ $rc -eq 0 ] && ( [ -z "$out" ] || [ "$out" = "{}" ] ); then
        pass "G39: l2_phase=frozen + final_report sentinel as last tool_use -> exit 0"
    else
        fail "G39: l2_phase=frozen + final_report sentinel as last tool_use -> exit 0 (rc=$rc, out=$out)"
    fi
}

run_g40() {
    require_source "$HOOK" "G40: l2_phase=frozen + write_code sentinel as last tool_use -> exit 0" || return
    local tmp out rc tp
    tmp="$(mktemp -d)"
    make_fixture "$tmp/t.jsonl" \
        '{"type":"user","message":{"role":"user","content":[{"type":"text","text":"test"}]}}' \
    ...
    rm -rf "$tmp"
    if [ $rc -eq 0 ] && ( [ -z "$out" ] || [ "$out" = "{}" ] ); then
        pass "G40: l2_phase=frozen + write_code sentinel as last tool_use -> exit 0"
    else
        fail "G40: l2_phase=frozen + write_code sentinel as last tool_use -> exit 0 (rc=$rc, out=$out)"
    fi
}
...
run_g41() {
    ...
    rm -rf "$tmp"
    if [ $rc -eq 0 ] && ( [ -z "$out" ] || [ "$out" = "{}" ] ); then
        pass "G41: l2_phase=done + no triggers -> exit 0"
    else
        fail "G41: l2_phase=done + no triggers -> exit 0 (rc=$rc, out=$out)"
    fi
}

run_g42() {
    require_source "$HOOK" "G42: l2_phase=null + final_report sentinel only -> exit 0 (exempt)" || return
    local tmp out rc tp
    tmp="$(mktemp -d)"
    make_fixture "$tmp/t.jsonl" \
        '{"type":"user","message":{"role":"user","content":[{"type":"text","text":"test"}]}}' \
    ...
    rm -rf "$tmp"
    if [ $rc -eq 0 ] && ( [ -z "$out" ] || [ "$out" = "{}" ] ); then
        pass "G42: l2_phase=null + final_report sentinel only -> exit 0 (exempt)"
    else
        fail "G42: l2_phase=null + final_report sentinel only -> exit 0 (exempt) (rc=$rc, out=$out)"
    fi
}
