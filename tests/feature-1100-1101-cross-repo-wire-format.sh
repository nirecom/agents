...
if [ "$RC" -eq 0 ]; then
    ...
    fail "C1: rc=$RC expected exit 0 for cross-repo entry nirecom/my-private-repo#42"
fi
teardown_tmp
...
# ============================================================================
# C3: wip-set-single.sh --repo nirecom/my-private-repo 42 → gh called correctly
#
...
# ============================================================================
# C4: issue-state-check.sh --repo nirecom/my-private-repo 42 → exit 0 + 'open'
#
...
if [ ! -f "$STATE_SCRIPT" ]; then
    ...
    GH_MOCK_SCENARIO=issue_task \
        OUT=$(run_with_timeout 30 bash "$STATE_SCRIPT" --repo nirecom/my-private-repo 42 2>/dev/null)
    RC=$?
    if [ "$RC" -eq 0 ] && [ "$OUT" = "open" ]; then
        pass "C4: issue-state-check.sh --repo nirecom/my-private-repo 42 → exit 0 + 'open'"
    else
