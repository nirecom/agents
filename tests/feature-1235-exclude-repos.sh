...
# EXCLUDE_REPOS=<parent>/my-specs-repo, repo is <parent>/my-specs-repo-old → must BLOCK
...
# Then test that the actual repo (my-specs-repo-old) is NOT excluded
...
if run_pre_commit "$REPO_B3" ENFORCE_WORKTREE=on \
    ...
    pass "Part B-3: SIBLING-PREFIX — my-specs-repo-old correctly blocked despite sibling my-specs-repo in EXCLUDE_REPOS"
fi
