...
#
# Hermetic by construction: every case drives the checker through its documented
# PRIVATE_REPO_NAMES_CACHE_SET / PRIVATE_REPO_NAMES_CACHE contract, or through a
# stand-in AGENTS-config directory whose hooks/lib/is-private-repo.js is a stub. No
# case reaches `gh`, the network, or the running user's real repo list — and no real
# private repo name appears anywhere in this file.
#
...
# - /worktree-start actually exporting the cache before it derives a name (that seam
#   is covered one layer up, in tests/feature-worktree-start-non-interactive/).
...
# run_check <cache> <candidate...> — drive the checker through the cache contract.
...
module.exports = { listPrivateRepoNames };
STUB
...
if printf '%s\n' "$ERR" | grep -qxF "$P2_MSG" && [ -z "$P5_LEAKED" ]; then
    pass "P5/fixed-diagnostic: the live path emits the same fixed literal and names neither the repo nor the candidate"
else
    fail "P5/fixed-diagnostic: expected '$P2_MSG' alone on the live path (leaked:$P5_LEAKED, err='$ERR')"
fi
