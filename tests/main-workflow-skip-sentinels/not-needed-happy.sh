...
echo ""
echo "=== W1: OUTLINE_NOT_NEEDED judgment=true prefix → outline=skipped, skip_reason verbatim ==="

SID="w1-jt-outline-$$"
write_state "$SID" "$(ALL_COMPLETE_EXCEPT outline "$SID")"

MARK_JSON=$(build_mark_json 'echo "<<WORKFLOW_OUTLINE_NOT_NEEDED: judgment=true: single obvious approach>>"' "$SID")
MARK_OUT=$(run_mark "$MARK_JSON")

expect_state_step "W1a. OUTLINE_NOT_NEEDED judgment=true → outline=skipped" \
    "$SID" "outline" "skipped"

W1_REASON=$(read_state_field "$SID" "outline" "skip_reason")
if [ "$W1_REASON" = "judgment=true: single obvious approach" ]; then
    pass "W1b. outline.skip_reason = 'judgment=true: single obvious approach'"
else
    fail "W1b. expected skip_reason='judgment=true: single obvious approach', got: $W1_REASON"
fi

echo ""
echo "=== W2: DETAIL_NOT_NEEDED judgment=true prefix → detail=skipped, skip_reason verbatim ==="

SID="w2-jt-detail-$$"
write_state "$SID" "$(ALL_COMPLETE_EXCEPT detail "$SID")"

MARK_JSON=$(build_mark_json 'echo "<<WORKFLOW_DETAIL_NOT_NEEDED: judgment=true: all files identified>>"' "$SID")
MARK_OUT=$(run_mark "$MARK_JSON")

expect_state_step "W2a. DETAIL_NOT_NEEDED judgment=true → detail=skipped" \
    "$SID" "detail" "skipped"

W2_REASON=$(read_state_field "$SID" "detail" "skip_reason")
if [ "$W2_REASON" = "judgment=true: all files identified" ]; then
    pass "W2b. detail.skip_reason = 'judgment=true: all files identified'"
else
    fail "W2b. expected skip_reason='judgment=true: all files identified', got: $W2_REASON"
fi
