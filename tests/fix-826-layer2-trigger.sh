...
# A1 (RED→GREEN): empty state → appendFinding() with valid finding → layer2.alert_armed_at is non-null ISO 8601 string
run_A1() {
    local tmp val rc
    tmp="$(mktemp -d)"
    WORKFLOW_PLANS_DIR="$tmp" run_with_timeout 5 node -e "
...
" >/dev/null 2>&1
    rc=$?
    val=$(read_field "$tmp" "a1-sid" "alert.alert_armed_at")
    rm -rf "$tmp"
    # appendFinding must return true AND layer2.alert_armed_at must be a non-null ISO 8601 string
    if [ $rc -eq 0 ] && [ "$val" != "null" ] && [ -n "$val" ] && [ "$val" != '""' ]; then
        pass "A1: empty state -> appendFinding() -> layer2.alert_armed_at is non-null ISO 8601 string"
    else
        fail "A1: empty state -> appendFinding() -> layer2.alert_armed_at is non-null ISO 8601 string (rc=$rc, val=$val)"
    fi
}

# A2 (RED→GREEN): state with alert_armed_at already set → appendFinding() → alert_armed_at unchanged (no-clobber)
run_A2() {
    local tmp val rc
    tmp="$(mktemp -d)"
...
" >/dev/null 2>&1
    rc=$?
    val=$(read_field "$tmp" "a2-sid" "alert.alert_armed_at")
    rm -rf "$tmp"
    # appendFinding must return true AND layer2.alert_armed_at must remain the original value (no-clobber)
    # The fix must set alert_armed_at when null but NOT overwrite an existing value.
    # To confirm no-clobber, the value must equal the seeded fixed_ts.
    # This test is RED→GREEN: currently appendFinding does not set alert_armed_at at all,
    # so the value stays "2026-01-01T00:00:00.000Z" — but we need to verify it alongside A1.
    # The RED condition: A1 must pass first (appendFinding sets alert_armed_at), which confirms
    ...
    # also sets alert_armed_at on this call (which means it should NOT, since one is already set).
    # We verify the value is EXACTLY the seeded value (not a new timestamp).
    if [ $rc -eq 0 ] && [ "$val" = "\"$fixed_ts\"" ]; then
        pass "A2: alert_armed_at already set -> appendFinding() -> alert_armed_at unchanged (no-clobber)"
    else
        fail "A2: alert_armed_at already set -> appendFinding() -> alert_armed_at unchanged (no-clobber) (rc=$rc, val=$val)"
    fi
}

# A3 (RED→GREEN, dedup path): state already contains same finding + alert_armed_at === null
#   → appendFinding() same finding → returns true AND alert_armed_at non-null
run_A3() {
    local tmp val rc
    tmp="$(mktemp -d)"
...
" >/dev/null 2>&1
    rc=$?
    val=$(read_field "$tmp" "a3-sid" "alert.alert_armed_at")
    rm -rf "$tmp"
    # On the dedup path, appendFinding returns true early (before writing).
    # The fix must ALSO set alert_armed_at even on the dedup path when it is null.
    if [ $rc -eq 0 ] && [ "$val" != "null" ] && [ -n "$val" ] && [ "$val" != '""' ]; then
        pass "A3: dedup path + alert_armed_at null -> appendFinding() -> returns true AND alert_armed_at non-null"
    else
        fail "A3: dedup path + alert_armed_at null -> appendFinding() -> returns true AND alert_armed_at non-null (rc=$rc, val=$val)"
    fi
}

# A3b (RED→GREEN, partial layer2): same as A3 but seed state has alert: {} (alert_armed_at key absent)
run_A3b() {
    local tmp val rc
    tmp="$(mktemp -d)"
...
" >/dev/null 2>&1
    rc=$?
    val=$(read_field "$tmp" "a3b-sid" "alert.alert_armed_at")
    rm -rf "$tmp"
    # With layer2:{} (alert_armed_at key absent → undefined → treated as null),
    # appendFinding must set alert_armed_at to a non-null ISO 8601 string.
    if [ $rc -eq 0 ] && [ "$val" != "null" ] && [ -n "$val" ] && [ "$val" != '""' ]; then
        pass "A3b: partial layer2:{} + alert_armed_at absent -> appendFinding() -> alert_armed_at non-null"
    else
        fail "A3b: partial layer2:{} + alert_armed_at absent -> appendFinding() -> alert_armed_at non-null (rc=$rc, val=$val)"
    fi
}
...
# (alert_armed_at still null). Documents the validation boundary is preserved post-fix.
run_A4() {
...
" >/dev/null 2>&1
    rc=$?
    val=$(read_field "$tmp" "a4-sid" "alert.alert_armed_at")
    rm -rf "$tmp"
