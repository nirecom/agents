...
# and RUN_TL3 guards together let the script reach the claude check.
...
# ============================================================================
# Case 1 — .env.example contains a RUN_TL3 entry and passes review-env-example
# ============================================================================
echo "=== Case 1: .env.example RUN_TL3 entry ==="

if [ ! -f "$ENV_EXAMPLE" ]; then
    fail "1. .env.example not found at $ENV_EXAMPLE"
else
    if grep -qE '^[[:space:]]*RUN_TL3=' "$ENV_EXAMPLE"; then
        pass "1a. RUN_TL3 entry present in .env.example"
    else
        fail "1a. RUN_TL3 entry missing from .env.example"
    fi
...
# ============================================================================
# Case 2 — phase5 script with FEATURE_644_PHASE=5 + RUN_TL3=off exits 77
# ============================================================================
echo "=== Case 2: phase5 + RUN_TL3=off ==="
...
if [ ! -f "$PHASE5_SCRIPT" ]; then
    ...
    set -e
    if [ "$rc" -eq 77 ]; then
        if grep -qiE 'RUN_TL3' /tmp/941-c2.out; then
            pass "2. exit 77 with RUN_TL3-related SKIP message"
        else
            pass "2. exit 77 (RUN_TL3=off honoured; SKIP message may differ)"
        fi
    else
        fail "2. expected exit 77 with RUN_TL3=off, got $rc"
...
# ============================================================================
# Case 3 — phase5 script with FEATURE_644_PHASE=5 + RUN_TL3=on passes guards
# ============================================================================
echo "=== Case 3: phase5 + RUN_TL3=on (guards pass) ==="

if [ ! -f "$PHASE5_SCRIPT" ]; then
    fail "3. phase5 script not found"
else
    set +e
    PATH="$STUB_DIR:$PATH" FEATURE_644_PHASE=5 RUN_TL3=on \
        run_with_timeout 60 bash "$PHASE5_SCRIPT" >/tmp/941-c3.out 2>&1
    rc=$?
    set -e
    # With both guards passed, the script must NOT exit 77 due to RUN_TL3 or FEATURE_644 guards.
    # It may still SKIP for downstream reasons (timeout, parse failure) — those skips emit
    # SKIP messages mentioning claude/response/parse, not the RUN_TL3/FEATURE guards.
    if [ "$rc" -eq 77 ]; then
        if grep -qiE 'requires RUN_TL3|requires FEATURE_644_PHASE' /tmp/941-c3.out; then
            fail "3. RUN_TL3=on still tripped the RUN_TL3/FEATURE guard (rc=77)"
            sed 's/^/  | /' /tmp/941-c3.out
        else
            pass "3. RUN_TL3/FEATURE guards passed (later SKIP from downstream cause)"
        fi
    else
...
# ============================================================================
# Case 4 — FEATURE_644_PHASE=4 + RUN_TL3=on → FEATURE gate fires first (exit 77)
# ============================================================================
echo "=== Case 4: FEATURE_644_PHASE=4 + RUN_TL3=on ==="

if [ ! -f "$PHASE5_SCRIPT" ]; then
    fail "4. phase5 script not found"
else
    set +e
    PATH="$STUB_DIR:$PATH" FEATURE_644_PHASE=4 RUN_TL3=on \
