#!/bin/bash
# tests/hooks/feature-1027-state-writer-eligibility.sh
...
# Tests for issue #1027 / #997 — writer accepts new fields and ensureAlertScheduled
...
if (st.alert.findings_surfaced_at !== '2026-06-21T02:00:00Z') { console.error('findings_surfaced_at not persisted'); process.exit(4); }
if (st.alert.alert_eligible_phase !== 'post_final_report_window') { console.error('alert_eligible_phase not persisted'); process.exit(5); }
" >/dev/null 2>&1
    rc=$?
    rm -rf "$tmp"
    if [ "$rc" = "0" ]; then
        pass "W1: writeAlertState persists findings_surfaced_at + alert_eligible_phase"
    else
        fail "W1: writeAlertState persists new keys (rc=$rc)"
    fi
}

# --- W2: anchor present + alert_eligible_phase=null -> arm SKIPPED (regression) -
run_w2() {
...
" >/dev/null 2>&1
    ...
    rm -rf "$tmp"
    if [ "$rc" = "0" ]; then
        pass "W2: anchor present + null eligibility -> alert_armed_at remains null"
    else
        fail "W2: anchor + null eligibility blocked arm (rc=$rc)"
    fi
}

# --- W3: anchor present + alert_eligible_phase=post_final_report_window -> arm proceeds ---
run_w3() {
...
" >/dev/null 2>&1
    ...
    rm -rf "$tmp"
    if [ "$rc" = "0" ]; then
        pass "W3: anchor + post_final_report_window eligibility -> arm fires (alert_phase=pending)"
    else
...
# --- W5: alert_phase=done overrides eligibility ----------------------------------
run_w5() {
...
" >/dev/null 2>&1
    ...
    rm -rf "$tmp"
    if [ "$rc" = "0" ]; then
        pass "W5: alert_phase=done overrides eligibility (no arm)"
    else
...
if (reread.alert.alert_eligible_phase !== 'post_final_report_window') { console.error('upcast value not present'); process.exit(4); }
" >/dev/null 2>&1
    rc=$?
    rm -rf "$tmp"
    if [ "$rc" = "0" ]; then
...
# --- W7: anchor absent + alert_eligible_phase=post_final_report_window -> arm fires ---
run_w7() {
...
" >/dev/null 2>&1
    ...
    rm -rf "$tmp"
    if [ "$rc" = "0" ]; then
        pass "W7: anchor absent + post_final_report_window eligibility -> arm fires (alert_phase=pending)"
    else
