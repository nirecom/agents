#!/usr/bin/env bash
# tests/hooks/unit-rules-policy-reader.sh
...
# Layer: TL2 (real node subprocess, real fixture policy files on disk; the parsers themselves are pure, but loadPolicyAsData reads real bytes and the canary cases need a real module body that a real require() would have executed). Dispatcher only — the cases live in tests/hooks/unit-rules-policy-reader/.
