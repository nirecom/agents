#!/usr/bin/env bash
# tests/hooks/cc-supervisor-emit-rules-injection.sh
# Tests: hooks/lib/supervisor-emit.js
# Tags: rules-injection, supervisor, supervisor-emit, instructions-loaded, TL2, scope:common
#
# TL2 coverage of the new reportRulesInjection() export (detail plan section 3-5). Unlike tests/hooks/feature-831-supervisor-emit.sh, which stubs appendFinding, this file writes through the REAL supervisor-state-writer into a pinned fixture plans dir and reads the persisted finding back — the plan's claim is about what actually lands in the state file, and a stubbed writer cannot fail on a validateFinding rejection. Layer: TL2 (real node subprocess, real state file in a fixture dir).
