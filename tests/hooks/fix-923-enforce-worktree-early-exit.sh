...
#
# Issue #923: early-exit checks isMainCheckout(earlyRoot)+isAllowedWorktreeCommand()
# before detectWritePredicate — security: CWD, not just -C, must be main worktree.
# NOTE(#838): linked CWD + -C main → ALLOW when same-repo series (T923-L2.E updated).
# L3 gap: hook registration not covered — see bin/check-verification-gate.sh (hook-registration).
...
# ── T923-L2.E: git -C <main> worktree remove /wt from linked CWD → ALLOW (#838)
# Pre-#838 this was BLOCK (security: CWD must be main, not just -C target).
# #838 revised: same-repo-series (shared git common-dir) linked CWD is a trusted
# peer — non-forceful worktree remove is ALLOW. --force → BLOCK (T923-L2.D);
# cross-repo -C → BLOCK (T923-L2.F).
L2E_OUT="$(run_bash_guard "git -C \"$MAIN_N\" worktree remove \"$TARGET_N\"" "$LINKED_WT" ENFORCE_WORKTREE=on)"
assert_allow "$L2E_OUT" "T923-L2.E: git -C <main> worktree remove /wt from linked CWD → ALLOW (#838)"
