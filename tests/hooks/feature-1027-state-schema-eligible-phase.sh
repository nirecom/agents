...
run_e1() {
    require_source "$SCHEMA_MODULE" "E1: createEmptyState exposes findings_surfaced_at + alert_eligible_phase" || return
    local rc
    run_with_timeout 10 node -e "
const s = require('$SCHEMA_MODULE_NODE');
const st = s.createEmptyState('sid-e1');
if (!('findings_surfaced_at' in st.alert)) { console.error('missing findings_surfaced_at'); process.exit(2); }
if (st.alert.findings_surfaced_at !== null) { console.error('findings_surfaced_at not null'); process.exit(3); }
if (!('alert_eligible_phase' in st.alert)) { console.error('missing alert_eligible_phase'); process.exit(4); }
if (st.alert.alert_eligible_phase !== null) { console.error('alert_eligible_phase not null'); process.exit(5); }
" >/dev/null 2>&1
    rc=$?
    if [ "$rc" = "0" ]; then
        pass "E1: createEmptyState exposes findings_surfaced_at + alert_eligible_phase (both null)"
    else
        fail "E1: createEmptyState exposes findings_surfaced_at + alert_eligible_phase (rc=$rc)"
    fi
}

# --- E2: validate accepts null and post_final_report_window for alert_eligible_phase ----
run_e2() {
    require_source "$SCHEMA_MODULE" "E2: validate accepts alert_eligible_phase null + post_final_report_window" || return
    local rc
    run_with_timeout 10 node -e "
const s = require('$SCHEMA_MODULE_NODE');
function mk(val) {
  const st = s.createEmptyState('sid-e2');
  st.alert.alert_eligible_phase = val;
  return st;
}
...
" >/dev/null 2>&1
    rc=$?
    if [ "$rc" = "0" ]; then
        pass "E2: validate accepts alert_eligible_phase null + post_final_report_window"
    else
        fail "E2: validate accepts alert_eligible_phase null + post_final_report_window (rc=$rc)"
    fi
}

# --- E3: validate rejects "normal_run" and other strings ---------------------
run_e3() {
    require_source "$SCHEMA_MODULE" "E3: validate rejects invalid alert_eligible_phase values" || return
    local rc
    run_with_timeout 10 node -e "
const s = require('$SCHEMA_MODULE_NODE');
function check(val) {
...
" >/dev/null 2>&1
    rc=$?
    if [ "$rc" = "0" ]; then
        pass "E3: validate rejects normal_run / bogus / integer for alert_eligible_phase"
    else
        fail "E3: validate rejects invalid alert_eligible_phase values (rc=$rc)"
    fi
}
...
function check(val) {
  ...
  st.alert.findings_surfaced_at = val;
  return s.validate(st).ok;
}
...
# --- E5: ALERT_ELIGIBLE_PHASE_VALUES export ------------------------------------
run_e5() {
    require_source "$SCHEMA_MODULE" "E5: ALERT_ELIGIBLE_PHASE_VALUES exported" || return
    local rc
    run_with_timeout 10 node -e "
const s = require('$SCHEMA_MODULE_NODE');
if (!Array.isArray(s.ALERT_ELIGIBLE_PHASE_VALUES)) { console.error('not array'); process.exit(2); }
const v = s.ALERT_ELIGIBLE_PHASE_VALUES;
...
" >/dev/null 2>&1
    rc=$?
    if [ "$rc" = "0" ]; then
        pass "E5: ALERT_ELIGIBLE_PHASE_VALUES equals [null, 'post_final_report_window']"
    else
        fail "E5: ALERT_ELIGIBLE_PHASE_VALUES export (rc=$rc)"
    fi
}
...
st.alert.findings_surfaced_at = '2026-06-21T01:23:45Z';
st.alert.alert_eligible_phase = 'post_final_report_window';
const raw = JSON.stringify(st);
const parsed = JSON.parse(raw);
if (parsed.alert.findings_surfaced_at !== '2026-06-21T01:23:45Z') { console.error('roundtrip findings_surfaced_at'); process.exit(2); }
if (parsed.alert.alert_eligible_phase !== 'post_final_report_window') { console.error('roundtrip alert_eligible_phase'); process.exit(3); }
