#!/usr/bin/env bash
# tests/hooks/cc-tl3-rules-injection-gate.sh
...
# The TL3 body itself is RUN_TL3-gated and skips (exit 77) on every ordinary run, so its decision table would otherwise never execute in CI. The table lives in tests/hooks/TL3-rules-injection-off-switch/helpers.sh (SSOT) and is sourced here — this file drives it directly with synthesized rc / observation / quiescence inputs, no claude subprocess and no filesystem. Layer: TL2 (sources the real gate helpers; pure function calls).
