# integration-post-compact/conv-lang-cases.sh — T12, T13, T15, T16, T17, T19, T20, T23
# Tests: hooks/post-compact.js, hooks/lib/conv-lang.js, hooks/session-start.js
# Tags: conv-lang, post-compact, scope:common
# Sourced by ../integration-post-compact.sh; inherits all variables and
# functions from helpers.sh. Cases run at source time, so the parent's source
# order IS the execution order. Owns the CONV_LANG injection contract for
# post-compact.js and its fail-open behaviour on malformed stdin.
...
# per call (symmetric with T18 for session-start; orthogonality CPR-ORTH).
# ===========================================================================
